...
// Функції tg-webhook і daily-reminder — застарілі, видалити.
//
// Після деплою прив'яжи вебхук (один раз, з браузера чи curl):
//   https://api.telegram.org/bot<ТОКЕН>/setWebhook?url=<SUPABASE_URL>/functions/v1/tg-commands
//
...
async function cmdSizes(chatId: number, target: { id: number; name: string }) {
  ...
  if (isFemale) {
    text += `\n<b>Нижня білизна</b>\n`;
    text += `Бюстгальтер: ${esc(sz.bra || "—")} / Труси: ${esc(sz.underwear || "—")}\n`;
  }
  text += `\n<b>Аксесуари</b>\n`;
