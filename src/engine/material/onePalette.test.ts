...
describe('one rose/amethyst palette (crystal cluster brief §6)', () => {
  ...
  it('separates faces by brightness alone', () => {
    ...
    /*
     * Золотий список рухався ДВІЧІ, і ось семантика обох рухів.
     *
     * (ADR-0120) Було `[0.73, 1, 1.18, 1.3]` у порядку циклу
     * `1.0 / 0.73 / 1.18 / 1.30`. Тон роздається за РАНГОМ грані в колі,
     * тож різниця між сусідами — це крок циклу: 27%, 62%, 10%, 23%. Два
     * світлі тони поспіль давали чверть усіх сусідніх пар різницю в
     * десять відсотків.
     *
     * Стало `1.16 / 0.68 / 1.38 / 0.78` — чергування світлого й темного,
     * кроки сусідів 41%, 103%, 44%, 49%.
     *
     * (ADR-0176) І ЦЕ Ж ЧЕРГУВАННЯ Й БУЛО ВАДОЮ, якої ADR-0120 не міряв.
     * У четвірці є друга відстань — пари ЧЕРЕЗ ОДНУ, (0,2) і (1,3). При
     * чергуванні це два світлі тони проти двох темних: 1.16 проти 1.38 —
     * 16%, 0.68 проти 0.78 — 13%.
     *
     * Око бачить саме таку пару там, де кожна друга грань вузька. Вінець
     * монарха має десять граней, у яких велика й вузенька йдуть
     * навпереміш (6–8 трикутників проти 1–3). Виміряно на живому кадрі:
     * прилад знаходив у вінці ОДНУ грань на 39 стовпців, 0.21–0.23 без
     * жодного кроку, тоді як на стовбурі — шість граней із кроками
     * 33–43%. Проба дикими тонами (0.2 проти 2.0) лишала вінець рівним,
     * бо при двох тонах усі непарні ранги — це один тон.
     *
     * Стало `0.68 / 0.975 / 1.4` — ТРИ тони, бо на колі з трьох пари
     * через одну збігаються з сусідніми, і збігу тонів не існує ні для
     * якого кроку ока. Чотири значення, попарно різні на 30%, у коридор
     * 0.66…1.5 не вміщаються: потрібне відношення 2.92, коридор дає 2.27.
     */
    const values = CRYSTAL_FACET_TINTING.tints.map((tint) => tint.r).sort((a, b) => a - b);
    expect(values).toEqual([0.68, 0.975, 1.4]);
  });
