...
function buildSubstrateMaterial(
  ...
  const bodyWithoutSignature: Omit<CrystalBodyMaterial, 'signature'> = {
    ...
    shader: {
      ...
      /*
       * 0.08 → 0.55 → 3.3, І ДРУГА ЦИФРА БУЛА ПРАВИЛЬНОЮ ВІДПОВІДДЮ НА
       * НЕПРАВИЛЬНЕ ЗАПИТАННЯ.
       *
       * Перше число (0.08) пережило геометрію, яку описувало: воно було
       * пораховане для широкої жили в 1.58 раза ширшої за колонію, що
       * займала нижню третину екрана (ADR-0061), а жеода сьогодні — 0.43
       * одиниці в радіусі. Виміряно наслідок: 0.71 плитки впоперек УСЬОГО
       * каменю, тобто менш ніж одна комірка на всю підкладку. Карта була
       * прикладена й не робила нічого; власник назвав це «паркан» тричі
       * поспіль (ADR-0139).
       *
       * 0.55 підняло це до 4.85 плитки на камінь — і камінь усе одно
       * лишився пласким, бо ОДИНИЦЯ ВИМІРУ БУЛА НЕ ТА. Камінь
       * гранований, і кожен його трикутник несе власну площинну
       * розгортку (`splitCrystalMeshFaces`). Візерунок, що повторюється
       * п'ять разів упоперек тіла, але менш ніж один раз усередині
       * КОЖНОЇ ГРАНІ, малює на кожній грані рівну пляму — просто щоразу
       * трохи іншу. Виміряно на 11 роках: 0.29 плитки на трикутник брили
       * при 4.85 на весь камінь. Власник побачив саме це.
       *
       * 3.3 дає медіанній грані брили 1.74 плитки замість 0.29. Засувка
       * `ЗЕРНО КАМЕНЮ ЧИТАЄТЬСЯ` міряє тепер ГРАНЬ, а не камінь.
       */
      surfaceTextureScale: round6(
        textureTier(SURFACE_CELLS_PER_UNIT * 3.3, CRYSTAL_MATERIAL_QUALITY_PRESETS[input.config.quality]),
      ),
