...
export interface CrystalShaderRecipe {
  ...
  /**
   * Наскільки світло всередині каменю ЗБИРАЄТЬСЯ ДО ВІСТРЯ.
   *
   * 0 — світло однакове по всій довжині; 1 — підніжжя вдвічі темніше за
   * середину, а вістря вдвічі світліше.
   *
   * ЦЕ ЗНАЙШОВ ФОТО-ЕТАЛОН. Власник надіслав знімок аметистової друзи, і
   * мірка (`scripts/live/photoProfile.mjs`) назвала головну різницю
   * числом: у знімку вістря на 0.43 світліше за підніжжя, у нас підніжжя
   * було на 0.22 світліше за вістря. Причина геометрична: внутрішнє
   * світло важиться тим, наскільки грань повернута до ока
   * (`facing³`), а грані головки нахилені від нього — тож вістря
   * гасло саме там, де в справжньому кристалі воно найяскравіше.
   *
   * Множиться САМЕ ВНУТРІШНЄ СВІТЛО, а не все вихідне. Різниця виміряна:
   * множення всього вихідного (`axialTintStrength`) забирає різницю між
   * сусідніми гранями разом із яскравістю — читаність граней падала з 45%
   * до нижче 30%, тобто кристал переставав читатись кристалом заради
   * градієнта, якого при цьому все одно не виходило.
   */
  tipGather: number;
  /**
