...
export function buildTreeFoliage(input: BuildTreeFoliageInput): TreeFoliageState {
  ...
  /*
   * РОЗДАЄТЬСЯ ПО КОЛУ, А НЕ ГІЛКА ЗА ГІЛКОЮ.
   *
   * Прохід перший був і лишається: по одному згустку на гілку, і саме він
   * тримає обіцянку «жодної придатної гілки без листя».
   *
   * Прохід другий раніше віддавав ГІЛЦІ ВСІ її слоти підряд — тобто гілка
   * з вісьмома слотами забирала всі вісім раніше, ніж наступна діставала
   * свій другий. Поки бюджет не кінчався, це нічого не значило. Щойно
   * кінчався — а він кінчається на всіх зрілих деревах, — обрізалось усе,
   * що стояло в хвості.
   *
   * У хвості стояли ПАГОНИ ЛІДЕРА. Виміряно: коли бюджет листя вичерпано,
   * обрізаються рівно 10 згустків, і всі десять — їхні; кожен пагін
   * лишається з ОДНИМ жмутком біля основи, тоді як тягнеться він на всю
   * півширину крони. На знімку сорокарічного дерева це читалось голими
   * сучками, що стирчать із крони. Там, де бюджету вистачало (активний
   * профіль на 12 роках), ті самі пагони мали по 2–3 згустки.
   *
   * Тобто будова, яку ADR-0112 додав саме щоб наповнити верх крони, першою
   * й голодувала — і рівно на тих деревах, де крона найбільша.
   *
   * Тепер слот за слотом: кожна гілка дістає другий згусток раніше, ніж
   * будь-яка — третій. Бюджет той самий, міняється лише те, кому дістається
   * останній згусток при нестачі: не «першим у списку», а «тим, у кого
   * найменше».
   */
  for (const entry of pending) {
    if (entry.slotCount > 0) emit(entry, entry.slotCount - 1);
  }
  const deepestSlot = pending.reduce((most, entry) => Math.max(most, entry.slotCount), 0);
  for (let step = 1; step < deepestSlot; step += 1) {
    for (const entry of pending) {
      const slot = entry.slotCount - 1 - step;
      if (slot >= 0) emit(entry, slot);
    }
  }
