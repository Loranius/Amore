...
describe('the root the whole colony grows out of (crystal cluster brief §4)', () => {
  ...
  it('ЗЕРНО КАМЕНЮ ЧИТАЄТЬСЯ: одиниця виміру — ГРАНЬ, а не камінь', () => {
    /*
     * ЦЕЙ ТЕСТ ЗАМІНЮЄ ВЛАСНУ ПОПЕРЕДНЮ РЕДАКЦІЮ ТОГО САМОГО ДНЯ, і
     * причину варто записати повністю, бо помилка була тонка.
     *
     * ADR-0139 знайшов, що карта зерна малювала рівну пляму: 0.71 плитки
     * впоперек УСЬОГО каменю. Виправлення підняло це до 4.85 і поставило
     * засувку на добуток розгортки геометрії й густини матеріалу — і
     * добуток був правильний, а от ОДИНИЦЯ ні.
     *
     * Бо камінь тут гранований, і кожен трикутник несе власну площинну
     * розгортку (`splitCrystalMeshFaces`). Отже візерунок, який
     * повторюється п'ять разів упоперек тіла, але МЕНШ НІЖ ОДИН РАЗ
     * усередині кожної грані, малює рівну пляму на кожній грані — просто
     * пляма щоразу трохи інша. Саме це власник і бачив: «камінці зверху
     * без текстури». Виміряно на 11 роках: 0.29 плитки на трикутник
     * брили при 4.85 на весь камінь.
     *
     * Тому одиниця виміру тепер — грань. Густина 0.55 → 3.3 від
     * кристалової; медіанна грань брили показує 1.74 плитки на
     * одинадцятому році замість 0.29.
     */
    for (const [years, count] of SIZES) {
      const { geometry, material } = colony(years, count);
      const root = geometry.meshes.find((mesh) => mesh.bodyId === CRYSTAL_SUBSTRATE_BODY_ID)!;
      const recipe = material.bodies.find((body) => body.bodyId === CRYSTAL_SUBSTRATE_BODY_ID)!;
      const density = recipe.shader.surfaceTextureScale;
      const uvs = root.uvs ?? [];
      expect(uvs.length, `${years}y підкладка має розгортку`).toBeGreaterThan(0);

      // Насип іде після шва, і саме його межа стереже: брили дрібніші за
      // шов, тож на них зерно зникає першим.
      const seam = root.profile.seamTriangleCount ?? 0;
      const perFace: number[] = [];
      for (let triangle = seam; triangle < root.indices.length / 3; triangle += 1) {
        let lowU = Number.POSITIVE_INFINITY;
        let highU = Number.NEGATIVE_INFINITY;
        let lowV = Number.POSITIVE_INFINITY;
        let highV = Number.NEGATIVE_INFINITY;
        for (let corner = 0; corner < 3; corner += 1) {
          const vertex = root.indices[triangle * 3 + corner]!;
        ...
        perFace.push(Math.max(highU - lowU, highV - lowV) * density);
      }
      expect(perFace.length, `${years}y насип є`).toBeGreaterThan(0);
      perFace.sort((first, second) => first - second);
      const median = perFace[Math.floor((perFace.length - 1) * 0.5)]!;

      /*
       * НИЖНЯ МЕЖА ПОЧИНАЄТЬСЯ З ЧЕТВЕРТОГО РОКУ, і це названа межа, а не
       * лазівка. `repeat` живе в одиницях світу, тож комірка має сталий
       * розмір: у першорічної пари вся жеода завширшки з ніготь, і одна
       * ціла комірка на грань там недосяжна, не перетворивши камінь
       * дорослої пари на луску. Виміряно на першому році: 0.58.
       */
      if (years >= 4) {
        expect(median, `${years}y плиток на грань брили`).toBeGreaterThan(1);
      }
      // Верхня — стара, записана причина: комірчаста карта, повторена
      // десятки разів на темному камені, читається зміїною шкірою.
      // Виміряно максимум 2.69 на сороковому році.
      expect(median, `${years}y зерно здрібніло до луски`).toBeLessThan(12);
    }
  });
