...
describe('the root the whole colony grows out of (crystal cluster brief §4)', () => {
  ...
  it('ЗЕРНО КАМЕНЮ ЧИТАЄТЬСЯ: карта міряється тим каменем, який є', () => {
    /*
     * Вада, яку це закриває, прожила три ADR і три зауваги власника
     * («паркан»), бо число було правильним для геометрії, якої вже
     * немає.
     *
     * `surfaceTextureScale` підкладки стояв на 0.08 від кристалової
     * густини. Обґрунтування було записане й було чесним: при
     * кристаловій густині візерунок повторювався разів двадцять п'ять
     * упоперек шва, а комірчаста карта, повторена так часто на темному
     * камені, читається зміїною шкірою. Але той шов був широкою жилою,
     * що займала нижню третину екрана (ADR-0061).
     *
     * Жеода сьогодні — 0.43 одиниці в радіусі. Виміряно: **0.71 плитки
     * впоперек усього каменю**, тобто менш ніж одна комірка. Карта була
     * прикладена й не робила НІЧОГО.
     *
     * Тому тест міряє не константу, а ДОБУТОК: розгортку самої геометрії
     * на густину матеріалу. Змінюється камінь — змінюється й число, і
     * межа ловить це замість людини.
     */
    for (const [years, count] of SIZES) {
      ...
      for (let slot = 0; slot < uvs.length; slot += 2) {
        low = Math.min(low, uvs[slot]!);
        high = Math.max(high, uvs[slot]!);
      }
      const tiles = (high - low) * recipe.shader.surfaceTextureScale;
      /*
       * Смуга широка НАВМИСНО, і причина фізична.
       *
       * `repeat` живе в одиницях світу, тож комірка зерна має сталий
       * розмір, а не сталу частку каменю. Отже дрібний камінь показує
       * МЕНШЕ комірок — і це правильно: у камінця не більше зерен, ніж у
       * брили, у нього їх менше. Виміряно 1.97 плитки на першому році,
       * 4.85 на одинадцятому, 12.36 на сороковому.
       *
       * Нижня межа боронить від того, що було: 0.71 плитки — це рівна
       * пляма замість каменю.
       */
      expect(tiles, `${years}y плиток упоперек каменю`).toBeGreaterThan(1.5);
      // Верхня — записана причина старого числа: за двадцять п'ять
      // повторень комірчаста карта на темному камені стає зміїною
      // шкірою. 18 лишає запас над виміряним максимумом 12.36 і не
      // доходить до тієї межі.
      expect(tiles, `${years}y зерно здрібніло до луски`).toBeLessThan(18);
    }
  });
