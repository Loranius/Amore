 ...
 *
 * **1.06 → 1.25, і причина геометрична (ADR-0158).** Власник зняв стелю
 * на кількість граней, призма стала дев'яти-десятигранною замість
 * шести-семигранної, і тіло СХУДЛО відносно оголошеного радіуса: у
 * шестикутника описане коло більше за вписане в 1.155 раза, у
 * дев'ятикутника — в 1.064. Кут многогранника, який доти додавав до
 * півширини п'ятнадцять відсотків, тепер додає шість.
 *
 * Оголошені числа при цьому НЕ переміряні вниз, і це свідомо. Вони не
 * лише описують тіло — на них стоїть посадка колонії, і саме той
 * надлишок шестикутника мовчки тримав просвіт між монархом і першою
 * дитиною. Переміряні вниз, вони давали перетин на 0.0036 (виміряно),
 * тобто ту саму ваду, від якої ADR-0125 і завів цю таблицю.
 *
 * Тому запас лишається запасом і названий числом: тіло вужче за
 * оголошене на чверть, і ця чверть — те, чим колонія дихає.
 */
const ALLOWED_SLACK = 1.25;
