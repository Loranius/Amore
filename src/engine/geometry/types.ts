...
export interface CrystalBodyProfile {
  ...
  /**
   * Найдальша точка КОНТУРУ — тієї кварцової плями, у якій стоять
   * кристали, — а не меша.
   *
   * Третій раз та сама наука, і щоразу на рівень глибше. Спершу
   * «найвища точка меша» перестала означати шов, бо на шов поклали
   * брили (`seamTriangleCount`). Потім «найвища точка шва» перестала
   * означати губу, бо по периметру встала стінка (`seamRimHeight`).
   * Тепер «найширша точка меша» перестала означати контур, бо ЗОВНІ
   * контуру став комір (ADR-0115), і він за побудовою ширший:
   * `collarRadius` бере відстань від монарха до контуру й множить її
   * на 1.22.
   *
   * Тест «жила — комір навколо дітей, а не калюжа під ними» міряв межу
   * меша й називав її жилою. Поки колонія була мала, комір відставав
   * недалеко, і число трималось; на тридцяти роках воно виходило за
   * власну межу — виміряно 1.4003 при порозі 1.35, і БЕЗ жодної зміни
   * в розкладці. Правило проти калюжі говорить про кварц під
   * кристалами; комір — це порода осторонь, і за те, як далеко вона
   * стоїть, відповідають засувки еталона (`rockSpread`).
   *
   * Підкладка, і лише нова.
   */
  seamOutlineRadius?: number;
  signature: string;
}
