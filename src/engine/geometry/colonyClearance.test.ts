...
describe('кільце років стоїть впритул, але не всередині монарха', () => {
  ...
  it('впритул — це саме впритул, а не кільце навколо', () => {
    ...
    /*
     * Просвіт — частка ширини самої дитини. Число рухалось двічі, і другий
     * рух вартий пояснення.
     *
     * Було 0.0178 при ширині близько 0.042 (42%), стало під 10% — і саме
     * ці 10% тут стояли межею. Але вони були виміряні під формулою, яка
     * обходила ОГОЛОШЕНИЙ радіус, а не справжню півширину тіла: на інших
     * парах та сама формула ставила дитину ВСЕРЕДИНУ монарха (ADR-0125,
     * знайдено розгорткою `crystalGrowthSweep`). Тобто «тіснота», яку це
     * число стерегло, частково була перетином.
     *
     * Відколи посадка обходить справжню півширину, просвіт тут 0.41 —
     * і це вже справжній просвіт між поверхнями. Кадр перевірено на
     * третьому й одинадцятому роках: колонія читається тісною купою
     * навколо монарха так само, як читалась.
     *
     * Межа лишається, бо її привід лишається: нікому не можна «полагодити»
     * тісноту, розсунувши колонію.
     */
    expect(closest / childWidth, 'діти відсунулись від монарха').toBeLessThan(0.5);
  });
});
