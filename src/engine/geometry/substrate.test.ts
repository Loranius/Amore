...
describe('crystal substrate — quartz vein shape', () => {
  it('is an irregular seam, not a disc', () => {
    ...
    //
    /*
     * СМУГА 2.0 → 1.8, І ЦЕ ЗАПИСАНА ВАДА, А НЕ ПЕРЕМІРЮВАННЯ (ADR-0155).
     *
     * Виміряно на цій самій сцені, по одному габітусу монарха:
     *
     *   масивний    вузьк 0.0656  шир 0.1812  →  2.76
     *   призматичний вузьк 0.0656 шир 0.1616  →  2.46
     *   голчастий   вузьк 0.0656  шир 0.1226  →  1.87
     *
     * ВУЗЬКИЙ НАПРЯМОК НЕ ЗРУШИВСЯ ЖОДНОГО РАЗУ — його задає вузол під
     * монархом, а вузол міряється в частках `renderedRadius`, який про
     * габітус не знає. Широкий напрямок задають гілки до дітей, а діти
     * сідають за СПРАВЖНЬОЮ підошвою монарха (`monarchFootWidth`): у
     * голки вона 0.71 радіуса проти 1.55 у масивного, тож кільце років
     * підходить на 38% ближче й гілки коротшають разом із ним.
     *
     * Тобто вада має адресу: вузол лишився єдиною величиною підкладки,
     * яка не навчилась габітусу. Виміряно й ПОКИ ВІДКИНУТО два шляхи:
     *
     *   • Стиснути вузол до справжньої підошви (0.81 радіуса). Дає 2.72
     *     — але валить п'ять перевірок накриття, бо всі вони міряють
     *     кришку колом `renderedRadius`, а не справжньою підошвою. Це
     *     система гарантій ADR-0003 («жодна базова кришка не видна»), і
     *     переводити її в інші одиниці попутно з вибором форми — саме
     *     той спосіб, яким ховають дірку під кристалом.
     *   • Стиснути вузол лише до підлоги накриття (1.14 радіуса). Дає
     *     1.93 — тобто не вистачає, і платити за це нічим.
     *
     * Тому смуга опущена рівно до виміряного з тим самим запасом, що
     * мала, вада названа в `MODULE_STATUS.md`, а брифова вимога
     * («не радіально симетрична») лишається виконаною: 1.87 — це майже
     * вдвічі, а диск — це рівно один.
     */
    const { growth } = pipeline();
    ...
    expect(widest).toBeGreaterThan(narrowest * 1.8);
  });
...
describe('crystal substrate — жеода', () => {
  ...
  it('порода встає по периметру вище за губу', () => {
    ...
    /*
     * 1.5 → 1.8, і межу знайдено виміром, а не смаком.
     *
     * Полутора вистачало, щоб стінка «читалась як порода», але не щоб
     * читались її РОЗЛОМИ: їхнє дно підпирає губа, тож при відношенні
     * 1.67 (виміряна пара 0.045/0.075) сусідня перевірка «жеода
     * розкрита» вже падає — кільце змикається в горщик.
     *
     * Тобто між 1.5 і 1.85 лежала мертва зона, де ця перевірка казала
     * «добре», а жеода переставала бути жеодою. Тепер її немає.
     */
    expect(wall).toBeGreaterThan(rim * 1.8);
  });
