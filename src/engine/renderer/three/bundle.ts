 ...
 */
/**
 * Зменшено 3.25 → 2.75 на прохання власника: «кристал виглядає громіздко,
 * занадто він великий на головному екрані; дерево і корал більш-менш
 * нормально на свій вік».
 *
 * ЧОМУ САМЕ ЦЯ РУЧКА, а не товщина. Спершу спробували зсунути смугу
 * товщини габітусів (×0.82, співвідношення збережені) — і це зламало
 * дві справжні геометричні гарантії, а не пороги:
 *
 ...
 *
 * Плюс `gemSilhouette` тримає смугу пропорції 3.4–4.35, і це не
 * випадкове число, а рішення власника від 2026-08-10, зроблене так само
 * з погляду на портал. Нинішні 3.41 стоять на самому нижньому краї тієї
 * смуги, тобто монарх уже настільки тонкий, наскільки дозволено.
 *
 * Ця ручка ні з чим не зчеплена: вона міняє лише те, скільки кадру
 * займає артефакт, бо камера кадрує за його висотою
 * (`portalCameraFrame`: `0.985 + 1.079·h`).
 *
 * Виміряно, у частках висоти екрана:
 *
 *   чотири роки   51% → 47%
 *   одинадцять    62% → 58%
 *   дерево        69% (не змінюється)
 *
 * Чому не нижче: `treeFit` тримає інваріант «дерево нижче за стелю
 * кристала» (2.7 < стеля). Перша спроба поставила 2.6 й перевернула його —
 * двадцятирічний кристал став би нижчим за дерево. 2.75 лишає відношення
 * цілим і віддає майже весь виграш.
 *
 * Кристал і далі лишається меншим за дерево на будь-якому віці — власник
 * сказав, що дерево на свій вік виглядає нормально.
 */
export const ARTIFACT_FIT_HEIGHT = 2.75;
