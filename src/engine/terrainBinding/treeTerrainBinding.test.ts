...
/*
 * ДВА ІНВАРІАНТИ ОКРУГЛЕННЯ ПЛАТО, І ВОНИ ТЯГНУТЬ У РІЗНІ БОКИ.
 *
 * Округлення вгору до межі кільця з'явилось в ADR-0090, щоб рівна земля
 * напевно накривала коріння. Виміряно згодом, що воно може з'їсти ВСЕ: на
 * `low` кілець чотири, і плато 0.241 округлювалось рівно в поверхню 0.289 —
 * за плато не лишалось жодного кільця, а край ґрунту ліпиться саме там.
 * Ґрунт ставав ідеальним колом (0.2894..0.2894 проти 0.2557..0.2864 на
 * `high`) — тим самим «гумовим килимком», проти якого край і робили.
 *
 * Обмеження ж не сміє опустити плато нижче за коріння, бо це та сама вада
 * ADR-0090 з протилежного боку. Тому тут перевіряються ОБИДВА кінці.
 */
describe('Tree Terrain Binding — округлення плато', () => {
  for (const lod of ['high', 'medium', 'low'] as const) {
    it(`${lod}: лишає кільце під край і не оголює коріння`, () => {
      const state = build(lod);
      expect(state.plateauRadius).toBeLessThan(state.surfaceRadius);
      expect(state.plateauRadius)
        .toBeGreaterThanOrEqual(state.diagnostics.rootCoverageRadius - 1e-6);
    });
  }
});
