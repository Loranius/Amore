import { add, normalize, round6, scale } from '../../growth/math';
...
function sampleCurveForLod(
  curve: OrganicBranchCurve,
  lod: OrganicMeshLod,
  config: OrganicSurfaceConfig,
): OrganicCurveFrameSample[] {
  const stride = Math.max(1, Math.floor(config.axialStrideByLod[lod]));
  const samples = curve.samples.filter(
    (_sample, index) => index === 0 || index === curve.samples.length - 1 || index % stride === 0,
  );
...
export function buildOrganicSweepMesh(
  ...
  for (const curve of frameState.curves) {
    const samples = sampleCurveForLod(curve, lod, config);
