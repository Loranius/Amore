 ...
 */
export const TREE_CROWN_HALF_WIDTH_SHARE = 0.425;
...
export function applyTreeCrownEnvelope(
  ...
  const nodes: OrganicSkeletonNode[] = skeleton.nodes.map((node) => {
     ...
     * піддерева).
     *
     * СПАД КВАДРАТИЧНИЙ, А НЕ ЛІНІЙНИЙ (ADR-0109). Чистий стовбур — це
     * найбільший залишок розриву з еталоном: у смузі 0.15-0.20 зросту наше
     * дерево дає 0.11 півширини там, де еталон дає 0.00, і дають це саме
     * чотири гілки-носії, що проходять крізь стовбур угору. Прибрати їх не
     * можна, а притиснути до осі — можна. Виміряно на сороковому році
     * (смуга 0.15-0.20 / відстань профілів):
     *
     *   лінійний   0.11 / 0.115
     *   квадрат    0.09 / 0.113      (взято)
     *   куб        0.08 / 0.111      — далі виграш дрібніє, а гілка
     *                                  притискається до кори
     *
     * Квадрат — та сама форма, якою сходить комель (`rootFlare.ts`): біля
     * землі притискає сильно, а до низу крони відпускає.
     */
    const cap = height * (heightShare < TREE_CROWN_BOTTOM_SHARE
      ? treeCrownHalfWidthAt(TREE_CROWN_BOTTOM_SHARE, narrowing)
        * (heightShare / TREE_CROWN_BOTTOM_SHARE) ** 2
