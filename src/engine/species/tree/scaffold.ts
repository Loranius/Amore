 ...
 *
 * ЗБІГ ДОГОРИ 0.45 -> 0.78, І ЦЕ ЗАКРИВАЄ ПЛАСКУ ВЕРХІВКУ (ADR-0106).
 *
 * Найвища скелетна гілка сидить на 0.88 зросту, і за збігу 0.45 вона все
 * ще вилітала на 0.209 — тобто дерево кінчалось ЗРІЗОМ завширшки в
 * половину крони. Виміряно по найвищій смузі силуету: наше 0.239 проти
 * еталонних 0.046.
 *
 * Розгортка збігу на сороковому році (найвища смуга / відстань профілів):
 *
 *   0.45 -> 0.239 / 0.129
 *   0.62 -> 0.147 / 0.109
 *   0.78 -> 0.093 / 0.105      (взято)
 *   0.90 -> 0.070 / 0.124      — верхівка вужча, а форма вже гірша
 *
 * За 0.90 крона починає сходити зарано й губить масу там, де еталон її ще
 * тримає; 0.78 найкраще і за верхівкою, і за формою загалом, причому на
 * всіх трьох перевірених віках одразу.
 */
function reachShareFor(index: number, narrowing: number): number {
  ...
  return TREE_SCAFFOLD_REACH_SHARE * (1 - 0.78 * (index / span)) * narrowing;
}
