...
/*
 * СКІЛЬКИ НАЙТОНШИХ ГІЛОЧОК ЗАМІЩАЮТЬ СІМ СКЕЛЕТНИХ — двадцять.
 *
 * Число не вибране, а виміряне проти БАЗОВОЇ ЛІНІЇ. На дев'яти віках × чотири
 * посіви активної пари, `medium`, найгірший меш:
 *
 *   без скелетних гілок узагалі: 19 591, порушень 1 з 36
 *   зі скелетами, відкинуто  0:  20 883, порушень 3 з 36
 *   зі скелетами, відкинуто 10:  20 320, порушень 2 з 36
 *   зі скелетами, відкинуто 20:  19 616, порушень 1 з 36   <-
 *   зі скелетами, відкинуто 30:  18 941, порушень 1 з 36
 *   зі скелетами, відкинуто 45:  18 161, порушень 1 з 36
 *
 * На двадцяти скелетні гілки стають БЮДЖЕТНО НЕЙТРАЛЬНИМИ: 19 616 проти
 * 19 591 базових, тобто дерево дістає сім справжніх гілок задарма. Далі
 * платить уже листя — середнє падає з 611 на 528 і 384, — а виграш у
 * трикутниках дрібніє.
 *
 * ПРО ТУ ЄДИНУ ПОРУШНИЦЮ. Вона була ДО цієї зміни: сорокарічна пара, що
 * жила всіма модулями, дає 19 591 при стелі 18 000 і без жодної скелетної
 * гілки. Тутешній тест її не бачив, бо перевіряв інші віки. Це окрема вада,
 * названа в ADR-0093 §6, і закривати її треба перерозподілом бюджету між
 * листям, коренями й деревиною, а не цим числом.
 */
const PRUNED_TWIGS_PER_SCAFFOLD_SET = 20;
 ...
 */
function pruneThinnestBranches(
...
export function addTreeScaffoldBranches(
  ...
  /*
   * СКЕЛЕТИ ЗАМІЩАЮТЬ НАЙТОНШУ ДРІБНОТУ, А НЕ ДОДАЮТЬСЯ ДО НЕЇ.
   *
   * Сім справжніх гілок коштують близько 1300 трикутників, і на
   * сорокарічній парі, що жила всіма модулями, це виводило меш за мобільну
   * стелю: 20 883 при 18 000. Стиснути стовбур було нічим — він уже стояв на
   * найгрубішому дозволеному кроці 8, за якого від сорока кілець лишається
   * п'ять.
   *
   * Тому найтонші гілочки симуляції тут ВІДКИДАЮТЬСЯ. Це не хитрість заради
   * бюджету, а те, що змінилось по суті: доки крону не було чим тримати,
   * кожен прутик був крона; тепер крону тримають скелетні гілки, а прутики —
   * наповнення. Відкидається найтонше, тобто рівно те, чого око на розмір
   * порталу й не бачить: гілочка радіусом 0.01 займає менше пікселя.
   *
   * Скільки саме — виміряно нижче за коментарем до `PRUNED_TWIGS_PER_SCAFFOLD_SET`.
   */
  const pruned = pruneThinnestBranches(skeleton, PRUNED_TWIGS_PER_SCAFFOLD_SET);
  return { ...pruned, nodes: [...pruned.nodes, ...added] };
}
