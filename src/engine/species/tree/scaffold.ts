...
export function addTreeScaffoldBranches(
  ...
  for (let index = 0; index < LEADER_SHOOTS; index += 1) {
    ...
    /*
     * ПАГОНИ ЛІДЕРА СТАЮТЬ НА ПОВНЕ КОЛО, а не на дугу.
     *
     * Тут стояло `(count + index) / (count + LEADER_SHOOTS)`, і намір був
     * правильний — не починати власного кола, щоб пагін не став точно за
     * скелетною гілкою. Але арифметика наміру не виконувала: при семи
     * гілках і восьми пагонах частки йдуть від 7/15 до 14/15, тобто всі
     * вісім пагонів лягають у дугу **168°**. Верх крони був
     * односторонній ЗА ПОБУДОВОЮ, і жоден посів цього не рятував.
     *
     * Виміряно на власній сітці крони рушія (8 секторів × 4 шари,
     * `canopyDepth.diagnostics.occupiedCellCount`): верхній шар займав
     * 1–6 секторів із восьми залежно від віку й профілю. Саме це на
     * знімку читалось як шов між кроною й лідером і як крона, що
     * розпадається на купи.
     *
     * Півкроку зсуву лишається від старого наміру й робить те, чого той
     * не зробив: пагін ніколи не стає точно за скелетною гілкою.
     */
    const azimuth = azimuthSeed + ((index + 0.5) / LEADER_SHOOTS) * Math.PI * 2
