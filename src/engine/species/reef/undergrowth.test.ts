...
describe('тон грані: дрібнота перестає бути папером', () => {
  ...
  it('тон кульки нікуди не зривається між сусідніми вершинами', () => {
    /*
     * ЩО ЦЕ НАСПРАВДІ СТЕРЕЖЕ — І ЧОМУ МЕЖА 0.45, А НЕ 0.3.
     *
     ...
     * (`scripts/live/edgeHardness.mjs`), і саме вона — присуд цієї роботи.
     *
     * Тут стоїть запобіжник проти ПОМИЛКИ, а не проти крутизни: маківка,
     * якій дали тон гребеня замість середнього по обводу, давала 34%
     * стрибок на рівному місці (ADR-0194 → ADR-0195), і саме така вада
     * має валити тест.
     *
     * 0.3 було знято тоді, коли тіло несло ОДИН градієнт — ребро. Тепер їх
     * два: ребро (19%) і тінь дотику при підошві (13%), і на одному ребрі
     * трикутника вони перемножуються. Виміряно: найгірша пара — борозна
     * нижнього кільця проти гребеня другого, 0.386. Це не вада, це дві
     * правильні величини в одному місці.
     */
