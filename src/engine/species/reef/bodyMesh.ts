 ...
 *
 * **8 → 12 (ADR-0193).** Вісім граней дають між сусідніми ребрами 45°, і
 * саме це власник назвав «крутими геометричними кутами»: корал виходив
 * гранованим олівцем, а не тілом. Дванадцять дають 30°, тобто силует
 * читається круглим, лишаючись низькополігональним.
 *
 * Ціна: тіло дорожчає з 80 трикутників до 120, тобто колонія
 * чотирирічної пари — з 5 120 до 7 680. Повернено з піщаного дна
 * (ADR-0193 §4), де ті самі трикутники лежали за серпанком.
 */
export const BODY_AZIMUTH_SEGMENTS = 12;
const AZIMUTH_SEGMENTS = BODY_AZIMUTH_SEGMENTS;
