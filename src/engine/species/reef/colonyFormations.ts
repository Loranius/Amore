 ...
 * і плани, і подорож.
 *
 * **0.40 → 0.15 (ADR-0182).** Це не косметика й не смак: число вирішує
 * ПОКРИТТЯ купола колоніями, бо радіус колонії прив'язаний до масштабу
 * голови, а не до її радіуса. Мірка (`reefProfile.ts`) показала, що при
 * 0.40 на четвертому році — а пара сьогодні там — колонії вкривали 22.4%
 * купола, тобто чотири п'ятих кадру були голою породою.
 *
 * Виміряно по всьому діапазону, покриття на четвертому році:
 *
 *   0.40 → 22.4%   0.25 → 28.1%   0.15 → 33.2%   0.05 → 39.8%
 *
 * Чому саме 0.15, а не менше. При 0.05 широта життя майже перестає
 * розширювати голову — тобто правило, заради якого це число існує, стає
 * мертвим. 0.15 лишає його живим і водночас підтягує пропорцію купола:
 * радіус на висоту 1.71 → 1.40, а сусідній коментар про `HEAD_RISE_SHARE`
 * прямо каже, що живий масив «ближчий до півкулі, ніж до тарілки» —
 * півкуля це 1.00.
 *
 * НАЗВАНА ЦІНА. Вужчий купол зводить колонії ближче одна до одної, а
 * їхні радіуси не змінюються, тож перетини пізніх років стають частішими:
 * на десятому році 3 → 4 пари, на двадцять п'ятому 32 → 43. Перший рік
 * із перетином лишається ШОСТИМ в усьому діапазоні — тобто ця зміна ту
 * ваду не вносить і не лікує, лише поглиблює вже записану (ADR-0182 §6).
 */
