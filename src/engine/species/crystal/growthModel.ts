...
/**
 * ПІВШИРИНА ТІЛА БІЛЯ ПІДОШВИ, у частках оголошеного радіуса.
 * ------------------------------------------------------------
 * Розгортка `crystalGrowthSweep` знайшла ваду, старшу за всі недавні
 * правки форми: перший річний кристал заходив в оболонку монарха з
 * нульового до п'ятого року на КОЖНОМУ профілі (−0.0011 на нульовому,
 * −0.0071 на третьому — 1.4% висоти тіла).
 *
 * Причина не в тісноті. `renderedRadius` — це НЕ півширина: геометрія
 * розтягує тіло обхватом габітусу (`geometry/habit.ts`), анізотропією
 * перерізу (`profileScales`) і кутом многогранника. Виміряно на живій
 * геометрії: монарх з оголошеним радіусом 0.0408 малюється завширшки
 * 0.0617.
 *
 * ОДНЕ ЧИСЛО НА ВСІХ НЕ ПРАЦЮЄ, і це теж виміряно. Множник 1.62 прибирає
 * перетин скрізь, але розсуває колонію там, де вона стояла правильно:
 * «впритул — це саме впритул» дає 0.66 при дозволених 0.15, і навіть
 * 1.29 дає 0.28. Бо справжня півширина йде від 0.71 (голка) до 1.79
 * (плита) — розкид у два з половиною рази.
 *
 * Тому таблиця, а не константа. Числа виміряні на побудованих тілах по
 * двадцять чотири насінини на габітус, і `bodyFootWidth.test.ts` у томі
 * геометрії доводить, що вони й далі правдиві. Це не дублювання таблиць
 * тому V: том II оголошує МЕЖУ, якою користується при посадці, а том V
 * показує, що в неї вкладається.
 *
 * БІЛЯ ПІДОШВИ, а не найширше місце тіла. Дитина стоїть на ґрунті й
 * торкається монарха нижньою третиною; нахил (`lean`) відносить вістря
 * убік на частку ДОВЖИНИ, і брати цей виліт у посадку означало б
 * розсунути колонію заради місця, де ніхто нікого не торкається.
 */
const MONARCH_FOOT_WIDTH: Readonly<Record<string, number>> = {
  prismatic: 1.27, massive: 1.55, needle: 0.71, tabular: 1.79,
};
const CHILD_FOOT_WIDTH: Readonly<Record<string, number>> = {
  prismatic: 1.27, massive: 1.34, needle: 0.99, tabular: 1.40,
  blade: 1.45, fan: 1.34, etched: 1.27,
};
/** Габітус, якого немає в таблиці, дістає найширший із відомих. */
const MONARCH_FOOT_WIDTH_DEFAULT = 1.79;
const CHILD_FOOT_WIDTH_DEFAULT = 1.45;

/** Півширина монарха біля підошви, у частках оголошеного радіуса. */
export function monarchFootWidth(archetype: string): number {
  return MONARCH_FOOT_WIDTH[archetype] ?? MONARCH_FOOT_WIDTH_DEFAULT;
}

/** Те саме для дитини: у неї інша таблиця перерізу, тож інші числа. */
export function childFootWidth(archetype: string): number {
  return CHILD_FOOT_WIDTH[archetype] ?? CHILD_FOOT_WIDTH_DEFAULT;
}
...
export function childDistance(input: {
  ...
  /** Габітус монарха — від нього залежить, наскільки він ширший за радіус. */
  monarchArchetype: string;
  /** Те саме для дитини. */
  ...
  /*
   * Обходиться СПРАВЖНЯ півширина обох тіл біля підошви, а не їхні
   * оголошені радіуси: див. `MONARCH_FOOT_WIDTH`.
   */
  const againstMonarch = input.monarchRadialScale * monarchFootWidth(input.monarchArchetype)
    + input.childRadialScale * childFootWidth(input.childArchetype)
...
export function coupleTint(
   ...
   *
   * Тож дуга від троянди (340°) через аметист до фіолету (270°) — рівно
   * та родина, яку §6 і називає. Індивідуальність лишається; скільки
   * саме відтінків на ній міститься, вирішує вимір, а не бажання —
   * див. `COUPLE_HUE_STEPS`.
   */
  /*
 ...
 *
 * Шість, а не дев'ять — і це наслідок вимірювання, а не смаку. Див. дугу
 * нижче: коли з неї пішов синій кінець, дев'ять щаблів на 70° стали
 * стояти по 0.068 один від одного, тобто перестали бути розрізнюваними.
 * Шість дають 0.114.
 */
const COUPLE_HUE_STEPS = 6;

/**
 * Дуга родини: 270° (фіолет) → 340° (троянда), у частках кола.
 *
 * Починалась із 220° (синій), і це була вада, знайдена виміром проти
 * денного неба. Небо світлої теми — зеніт `#7fb8e6`, серпанок `#bcdcf3`;
 * відстань відтінку до найближчого з них:
 *
 ...
 *
 * Тобто пара, чия дата випала на синій кінець, отримала б кристал, який
 * на денному небі майже не видно, — при тому що вночі він виглядав би
 * нормально. Контраст за яскравістю тут не рятує: усі відтінки лежать у
 * смузі 1.1–1.3 до неба, розділяє їх лише хроматика.
 *
 * §6 брифу веде туди ж незалежно: родина названа
 * «трояндово-аметистовою», і 220° — це не аметист.
 *
 * Після зсуву найближчий до неба відтінок стоїть на 0.367.
 */
/*
 * ДУГА ЗСУНУТА НА 10° ДО ФІОЛЕТУ (2026-09-07), і причина — надіслане фото.
 *
 * Власник надіслав знімок аметистової друзи й попросив «повтори вигляд».
 * Мірка того знімка тією ж функцією, що й нашого кадру, дала 294°; його
 * дата (2022-12-26) стояла на щаблі 3 із шести, тобто 312° — рожевий бік
 * родини. Зсув початку дуги з 270 на 260 ставить той самий щабель на
 * 302°, що в кадрі читається як 294°.
 *
 * Що НЕ змінилось: ширина дуги (70°), кількість щаблів (6) і відстань між
 * сусідніми — тобто правило «шість розрізнюваних відтінків від дати»
 * лишається чинним, і жодна пара не втрачає власного кольору.
 *
 * Синій кінець перевірявся окремо: 235° був відкинутий як «майже колір
 * денного неба» (відстань 0.19), 270° мав 0.37. 260° лежить між ними
 * ближче до безпечного — 0.32.
 *
 * Названо вголос: це підгонка родини під ОДНУ пару. Власник обрав саме
 * її, коли вибирав між «аметист для вас» і «аметист для всіх» (друге
 * скасувало б шість щаблів).
 */
const COUPLE_HUE_ARC_START = 260 / 360;
const COUPLE_HUE_ARC_SPAN = 70 / 360;
...
const COUPLE_TINT_SATURATION = 0.9;
