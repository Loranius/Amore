...
/*
 * ЩО ТУТ НЕ СТОЇТЬ І ЧОМУ — множник справжньої півширини.
 * ------------------------------------------------------------
 * Розгортка `crystalGrowthSweep` знайшла ваду, старшу за будь-яку з
 * недавніх правок форми: перший річний кристал заходить у монарха на
 * КОЖНОМУ профілі з нульового до п'ятого року (виміряно −0.0011 на
 * нульовому, −0.0071 на третьому при висоті 0.5077).
 *
 * Причина не в тісноті. `radialScale` — це НЕ півширина тіла: виміряно на
 * живій геометрії, монарх з оголошеним радіусом 0.0408 малюється
 * завширшки 0.0617, тобто **в 1.51 раза ширший за те число, за яким його
 * обходять**. Множників три, і кожен у своєму томі: обхват габітусу
 * (`geometry/habit.ts`, до 1.30), анізотропія перерізу (`profileScales`,
 * до 1.18) і кут многогранника (близько 1.05).
 *
 * Спробу закрити це одним множником на обидва радіуси ВІДКИНУТО виміром.
 * Він прибирає перетин на всіх віках (просвіт стає +0.013…+0.031), але
 * розсуває колонію там, де вона й так стояла правильно: тест «впритул —
 * це саме впритул» дає 0.28 при дозволених 0.15 навіть на 1.29, а на 1.62
 * — 0.66. Дві фікстури дають протилежні вироки, і це саме означає, що
 * помилка залежить від пари: кому які габітуси випали.
 *
 * Отже це не число, а КОНТРАКТ: опублікований радіус мусить означати
 * півширину, або кожен, хто його читає, мусить знати множник свого тіла.
 * Це окрема робота (зачіпає ще й `baseCoverOf` в ADR-0003), і поки її не
 * зроблено, `crystalGrowthSweep` тримає перетин під межею, щоб він не
 * виріс.
 */
/**
