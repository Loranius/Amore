  ...
  it.each(AGES)('на %s жодна дитина не сягає монарха', (asOf) => {
    for (const perYear of DENSITIES) {
      ...
      for (const child of crystal.formations) {
        ...
        /*
         * ЗАПАС ПЕРЕСТАВЛЕНО З 0.4 НА 0.5 (ADR-0150), і це та сама
         * рука, що ставила 0.4.
         *
         * 2026-08-10 власник знизив частку 0.5 → 0.4: половина
         * читалась як «майже такий самий». 2026-09-07 він же назвав
         * результат «просто стовпом, який стирчить із землі» й обрав
         * «кластер із нижчим монархом».
         *
         * Це не скасування першого рішення, а зняття умови, за якої
         * воно приймалось: тоді монарх був стрункий (5:1 номінально), і
         * дитина в половину його висоти справді читалась другим
         * монархом. Тепер монарх кремезний (4.0 номінально, силует
         * 2.50 при еталонних 2.51), тож він переважає ОБ'ЄМОМ, і
         * висока дитина поруч читається кристалом у скупченні, а не
         * суперником. Дві зміни — одна зміна: нарізно жодна з них не
         * дає того, що обрав власник.
         *
         * Виміряно на цих же історіях: найвища дитина сягає 0.441
         * монарха. В еталонному скупченні другий за висотою тримає
         * 0.552 — тобто ми й далі стриманіші за еталон.
         */
        expect(child.axialScale / monarch).toBeLessThanOrEqual(0.5 + 1e-9);
      }
    }
  });
