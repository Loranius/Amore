 ...
 *
 * 4 500 → 4 900 (ADR-0165): море хмар із 18 пасом стало 46, бо в еталоні
 * власника воно ТЛО, а не смуга на обрії. Вартість 4 808, тобто 98%.
 *
 * 4 900 → 5 950 (ADR-0166): зелень. Меш трави виріс зі 186 трикутників до
 * **1 224**: 518 мохових латок на плато, 256 звисів із кромки, 154 на
 * шапках летючих брил, 110 на кущах, решта — кущики. Це найдорожча одна
 * зміна цієї сцени після самого острова, і платимо ми за головну
...
export interface PortalPalette {
   ...
   *
   * ЖИВЕ В ПАЛІТРІ, А НЕ В CSS, і це та сама причина, з якої тут живе
   * туман: небо мусить зустрітися з ним в один колір, інакше на лінії, де
   * далина тане в туман, з'являється шов. Дві копії одного кольору
   * розходяться того дня, коли хтось поправить одну.
   *
   * `skyGlow` — тепла смуга там, де стоїть низьке сонце. Вона НЕ є
   * джерелом світла: §10 забороняє жовте світло на артефакті, і всі
   * чотири джерела (`keyColour`, `rimColour`, `skyLight`, `rootLight`)
   * лишились холодними. Тепле тут тільки ТЛО — так само, як у справжній
   * вечірній сцені небо за спиною тепле, а світло на камені однакове.
   */
  /**
   * ГОРИЗОНТ ЛЕЖИТЬ НАД ВЕРХНІМ КРАЄМ КАДРУ, і від цього залежить порядок
   * зупинок (ADR-0165).
   *
   * Камера дивиться вниз під 23.6° (`EYE_ELEVATION_SIN` 0.4), а половина
   * поля зору по вертикалі — 21°. Отже промінь горизонту виходить за
   * верхній край на 2.6°: УСЕ, ЩО ВИДНО В КАДРІ, ЛЕЖИТЬ ПІД ГОРИЗОНТОМ.
   * Верх кадру — це далина під самим горизонтом, низ — стрімкий погляд у
   * порожнечу під островом.
   *
   * Тому далеке стоїть УГОРІ кадру, а туман фарбує далеке у `fog`. Небо
   * вгорі мусить бути тим самим `fog`, інакше далина тане в один колір
   * на тлі іншого — і це не здогад: кадр показав розрив дальніх хмар із
   * небом 58.6 з 255 проти 44.9 у ближніх, тобто далина читалась гучніше
   * за близину. Було навпаки — фіолет угорі, тепло внизу.
   *
   * `skyDeep` — той кінець градієнта, що лежить НИЗУ кадру: найглибше
   * повітря під островом.
   */
  skyDeep: string;
  skyMid: string;
  skyGlow: string;
...
export const PORTAL_PALETTES: Record<'light' | 'dark', PortalPalette> = {
  ...
  light: {
     ...
     */
    /*
     * ТУМАН СТАВ ТЕПЛИМ (ADR-0165), і це не порушення §10.
     *
     * §10 забороняє ЖОВТЕ ДЖЕРЕЛО на артефакті, і `portalLighting.test.ts`
     * перебирає рівно чотири: корінь, ключ, заливку й небо-як-світло. Усі
     * чотири лишились холодними. Туман — не джерело: він каже, якого
     * кольору ПОВІТРЯ між оком і далиною, і на вечірньому небі воно тепле.
     *
    ...
    /*
     * КАМІНЬ УДЕНЬ ПОТЕПЛІШАВ (ADR-0165): `#b0a4b8` → `#b6a698`.
     *
     * Бузковий камінь під блакитним небом був цілісним; під вечірнім він
     * читається холодною плямою в теплому кадрі — тобто рівно тим швом,
     ...
     *
     * ТЕПЛІШИЙ ЩЕ НА КРОК (ADR-0169), бо власник попросив теплий храм, а
     * тепло тут можна дати лише кольором: сцена намальована, другого
     ...
     *
     * Тут стояло '#5f8a72' — прохолодний зелений, підібраний, поки
     * зелень була тридцятьма вісьмома кущиками на 0.04% кадру. Щойно
     * зазеленіла сама земля, той самий колір став м'ятним брезентом,
     * накинутим на камінь.
     *
     * Виміряно проти еталона власника. Найзеленіший відсоток його
     * острова — rgb(168,141,82): ЧЕРВОНОГО ТАМ БІЛЬШЕ ЗА ЗЕЛЕНИЙ, а
     * перевага зеленого над середнім із двох інших — лише 15.9. У нас
     * було rgb(97,147,119) і перевага 38.8, тобто вдвічі з половиною
     * зеленіше й помітно холодніше.
     *
     * Причина не в смаку, а в тому, що сцена НАМАЛЬОВАНА: в еталоні
     * листя освітлене помаранчевим заходом і виходить оливковим, а в нас
     * світла немає, тож захід треба запекти в сам колір. Удень трава
     ...
     *
     * Тут стояло '#fbe3cd' — «хмари ловлять те саме низьке сонце, що й
     * туман». Це правда про світло і хиба про КАДР: після того, як небо
     * стало теплим саме там, де стоїть море хмар, хмара кольору неба
     * майже перестала існувати: середня відстань від тла 17.1 з 441 у
     * кольорі проти 25.0 у прохолодної при тій самій площі 3.2% кадру.
     *
     * У вечірньому небі хмара й не буває кольору неба: сонце світить їй
     * у бік, а низ лишається в тіні й читається прохолодно-сірим. Саме
  ...
  dark: {
    fog: '#2a2038',
    // Ніч теж має низьке світло — місяць на горизонті, а не сонце. Тепле
    // тут не з'являється: воно перетворило б ніч на другий захід.
    skyDeep: '#150f24',
    skyMid: '#2a2044',
    skyGlow: '#4a3a63',
    /*
    ...
    driftRock: '#5a4c85',
    /*
     * УНОЧІ ЗЕЛЕНЬ ПРИГЛУШЕНА, І ЦЕ ВИМІРЯНЕ ВИПРАВЛЕННЯ (ADR-0166).
     *
     * Тут стояло '#6d9c86' із поясненням «камінь темний, і трава ловить
     * місяць». Пояснення правильне для тридцяти восьми кущиків і хибне
     * для покриву: щойно зазеленіла сама земля, острів засвітився
     * смарагдовим — і перемагав артефакт, тобто рівно те, що §10
     * забороняє.
     *
     * ЧОМУ ПАЛІТРА ПРО ЦЕ НЕ СКАЗАЛА. Порівнювали світлість у палітрі, а
     * на екран вони йдуть різними шляхами: камінь несе ще й карту зерна,
     * яка його гасить, а трава — ні. Виміряно на живому кадрі: при
     * '#6d9c86' мох давав 100 світлості проти 48 у каменю, тобто вдвічі
     * ярче; удень те саме відношення 0.8, тобто мох ТЕМНІШИЙ за камінь.
     *
     * 0.64 від колишнього кольору зводить нічне відношення до ~1.3:
     * зелень видно на темному камені, і вона не забирає кадр в
     * артефакта.
     */
    flora: '#466456',
