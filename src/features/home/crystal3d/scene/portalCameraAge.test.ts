...
describe('кристал на екрані росте разом із парою', () => {
  ...
  it('різниця між трьома й десятьма роками помітна оком', () => {
     ...
     *
     * **0.08 → 0.025 (ADR-0164), і це не послаблення вимоги, а зміна її
     * одиниці.** Власник показав еталон — літаючий острів, кристал
     * маленький посеред нього — і обрав «відвести камеру до острова»:
     * висота кадру зросла в 3.2 раза. Кристал через це займає втричі
     * меншу частку екрана в БУДЬ-ЯКОМУ віці, тож і різниця між віками
     * ділиться на ті самі 3.2. Дванадцять пунктів стали 3.5, а межа 0.08
     * стала 0.025 — та сама вимога в новому масштабі.
     *
     * Що НЕ змінилось: різниця й далі росте афінно, бо обидва множники
     * кадру помножені однаково.
     */
    const three = screenShare(build(3, 40), PHONE);
    const ten = screenShare(build(10, 40), PHONE);
    expect(ten - three).toBeGreaterThan(0.025);
  });

  it('молодий кристал не губиться в залі', () => {
    /*
     * Межа з іншого боку, і вона теж із історії: стала висота кадру вже
     * була, і трирічна пара займала 23% екрана. Тому висота кадру афінна,
     * а не стала.
     *
     * **0.35 → 0.11 (ADR-0164)**, і причина та сама: кадр тепер іде за
     * островом, тож кристал у ньому маленький за задумом. Виміряно 0.139
     * на першому році. Межа лишається тим, чим була: числом, нижче за яке
     * артефакт перестає бути видимим об'єктом, — просто в новому кадрі.
     */
    expect(screenShare(build(1, 40), PHONE)).toBeGreaterThan(0.11);
  });
});
