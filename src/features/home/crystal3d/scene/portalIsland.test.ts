...
describe('море хмар', () => {
  it('стоїть далі за все інше, інакше воно опиняється за островом', () => {
    /*
     * Промінь, що проходить над дальнім краєм острова, мав нахил близько
     * 14° від горизонталі; усе, що нижче, затуляло плато, тож хмара ближче
     * за п'ятнадцять радіусів просто зникала за каменем.
     *
     * **ТЕ ЧИСЛО БІЛЬШЕ НЕ ЧИННЕ (ADR-0165).** Воно рахувалось для камери
     * під вісім градусів; власник відвів кадр до острова, камера піднялась
     * на 23.6° (ADR-0164), і з тієї висоти видно набагато далі за кромку.
     * Дванадцять радіусів були наслідком кута, а не властивістю сцени.
     *
     * Що лишилось інваріантом: хмара не має права стати МІЖ оком і
     * островом. Це та сама вимога, що в брил, і міряється вона тим самим
     * кільцем камери — числом, яке не може розійтись саме з собою.
     */
    ...
    expect(closest).toBeGreaterThan(PORTAL_CAMERA_RING * 1.25);
  });
