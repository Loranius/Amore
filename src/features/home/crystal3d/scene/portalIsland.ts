 ...
 * РІВНІ. Тому вони стоять за 17–39 радіусів і на 2.6–4.8 нижче за плато —
 * так їхній силует лягає трохи вище за край острова, тобто в небі, а не
 * на землі. Ближче — і хмара опиняється рівно за островом, тобто ніде.
 *
...
export function buildPortalCloudGeometry(seed: number, count: number): THREE.BufferGeometry {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const width = 4.2 + seededUnit(seed, `${tag}:width`) * 6.6;
    const height = 0.9 + seededUnit(seed, `${tag}:height`) * 1.4;
    const cx = Math.cos(angle) * reach;
    const cz = Math.sin(angle) * reach;
    ...
    /*
     * Три горби, а не прямокутник. Прямокутна хмара читається смугою
     * туману; горби дають той самий силует, який кожен упізнає, і
     * коштують дванадцять трикутників.
     */
    const lobes = 5;
