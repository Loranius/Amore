...
describe('portal environment cost', () => {
  it('accounts for every object the environment draws', () => {
    // Печера малює рівно п'ять: камінь стін, підлогу, диск розлому,
    // промінь із нього й друзу. Було тринадцять — храм, колонада, декор
    // і небо.
  ...
  it('стеля не задерта: реальна вартість близька до неї, а не вдесятеро менша', () => {
    /*
     * Стеля, яку ніхто не дістає, — не межа, а дозвіл. Виміряно 8 278
     * трикутників на найдорожчому профілі при стелі 9 500, тобто запас
     * 13%. Якщо колись стане вдвічі менше — стеля мусить опуститись
