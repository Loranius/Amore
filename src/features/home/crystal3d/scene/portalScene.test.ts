...
describe('portal camera frame', () => {
  ...
  it('zooms in on a young crystal and pulls back from a grown one (ADR-0018)', () => {
    ...
    for (const aspect of [0.46, 1.6]) {
      ...
      /*
       * І в будь-якому віці він лишається ВИДИМИМ об'єктом кадру.
       *
       * **0.38 → 0.11 (ADR-0164), і це не послаблення, а зміна кадру.** До
       * ADR-0018 трирічна пара займала 23% висоти екрана, і 0.38 стерегло
       * саме це — «кристал не має губитись». Власник показав еталон
       * (літаючий острів, кристал маленький посеред нього) і обрав
       * «відвести камеру до острова»: висота кадру виросла в 3.2 раза, тож
       * ТА САМА вимога в новому кадрі — це 0.38 / 3.2 ≈ 0.12. Виміряно
       * 0.131 на наймолодшому тілі списку.
       *
       * Слово «головним» із цього рядка прибрано свідомо: головним у кадрі
       * тепер є острів, і це рішення власника, а не наслідок арифметики.
       */
  ...
  it('aims at the island, and never above the artifact’s tip', () => {
    /*
     * Ціль була сталою 1.25 світової одиниці — вище за верхівку трирічного
     * кристала (1.18), тобто камера цілилась у порожнечу над ним. Це й досі
     * заборонено.
     *
     * **ЩО ЗМІНИЛОСЬ (ADR-0164):** ціль опущена ще й на пів острова, бо
     * власник відвів кадр до нього. Острів висить ПІД площиною кристала, і
     * камера, що цілиться в кристал, кладе весь його корінь у нижню
     * третину кадру. Тому нижня межа тепер не «вище за третину висоти
     * артефакта», а «на оголошене опускання нижче за неї».
     */
    for (const height of [0.6, 1.18, 2.5, 3.25]) {
      ...
      // Не вище за верхівку — та сама вада, з якої цей тест народився.
      expect(above, `${height}`).toBeLessThan(height);
  ...
  it('sizes the frame at the artifact’s near side, not at its axis', () => {
     ...
     *
     * **АСПЕКТ 0.42 → 0.30 (ADR-0164).** Перевіряти це можна лише там, де
     * кадр справді тримає ШИРИНА, а висота кадру виросла в 3.2 раза разом
     * із відходом камери до острова: на 0.42 з тим самим артефактом тепер
     * виграє висота (11.0 проти 10.05), і рівність міряла б не те. 0.30 —
     * найвужчий аспект, який приймає сам кадр.
     */
    const narrowScreen = portalCameraFrame(0.3, 1.5, 1.2);
    expect(narrowScreen.distance).toBeCloseTo(
      (1.5 * 2 * 1.08) / (2 * Math.tan((42 / 2) * (Math.PI / 180)) * 0.3),
      6,
    );
  });
...
describe('portal environment cost', () => {
  it('accounts for every object the environment draws', () => {
    // Острів малює рівно сім: сам острів разом з уламками, храм, брили в
    // небі, рослинність, море хмар, водоспади й світляне кільце. Печера
    // малювала п'ять, руїна з колонадою — тринадцять.
    //
    // 4 → 5 (ADR-0163), і п'ятий call куплено за КОЛІР, а не за геометрію:
    // вершинний колір множиться на колір матеріалу, тож трава в меші каменю
    // вийшла б кольору «камінь × зелень». Зате меш один на обидва місця —
    // і плато, і летючі брили.
    //
    // 5 → 6 (ADR-0167) — водоспади, і шостий call куплено за ПРОЗОРІСТЬ:
    // вода малюється напівпрозорою й без запису глибини, а в меші каменю
    // це означало б напівпрозорий острів. Власник дозволив +3 call'и разом
    // із бюджетом трикутників (ADR-0164); це другий із трьох.
    //
    // 6 → 7 (ADR-0168) — світляне кільце, і сьомий call платиться за
    // ДОДАВАННЯ: інший режим змішування — це властивість матеріалу.
    // Третій із трьох дозволених; вільних більше немає.
  ...
  it('лишається під стелею для КОЖНОЇ пари й на кожному профілі', () => {
     ...
     *
     * Ні печера, ні острів так не влаштовані: скільки уламків лежить на
     * плато й скільки брил висить у небі — вирішує профіль якості, а куди
     * саме вони стають, вирішує насіння пари. Рівності більше не існує, і
  ...
  it('стеля не задерта: реальна вартість близька до неї, а не вдесятеро менша', () => {
    /*
     * Стеля, яку ніхто не дістає, — не межа, а дозвіл. Саме це й сталося
     * з островом: сцена подешевшала з 8 488 до 3 318 трикутників (друза
     * по стінах з'їдала 78% старої стелі), і стеля 9 500 стерегла б
     * порожнечу. Тому вона опустилась до 4 200 разом із вартістю.
     */
