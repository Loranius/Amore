...
// кристала нижче 20% висоти монарха. Референс навпаки має кілька коротких
// передніх уламків, тому один найменший не-hero dominant стабільно стає
// 18%-м базальним шпилем. Growth State та ключ не змінюються.
// ============================================================
...
export function ensureVisibleReferenceAccent(
  ...
  const position = monarchPosition
    .addScaledVector(monarchAxis, monarch.height * 0.018)
    .addScaledVector(radial, hostRadius * 0.7)
    .addScaledVector(tangent, hostRadius * 0.1);
  const direction = monarchAxis
    .clone()
    .multiplyScalar(0.48)
    .addScaledVector(UP, 0.18)
    .addScaledVector(radial, 0.94)
    .normalize();
