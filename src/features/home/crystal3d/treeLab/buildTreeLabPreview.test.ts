...
describe('Tree production preview pipeline', () => {
  ...
  /*
   * ЯВНА СТЕЛЯ ЧАСУ, БО ТЕСТ БУДУЄ 32 ДЕРЕВА, А НЕ ОДНЕ.
   *
   * Вісім віків на чотири зерна, і кожна збірка проходить увесь конвеєр.
   * Виміряно (medium, 4 зерна на вік, середнє): рік — 90 мс, 5 років —
   * 123 мс, 10 — 174 мс, 15 — 170 мс. Тобто 32 збірки — це близько 4-5 с, і
   * стандартні 5 с тест перевищував не через ваду, а тому, що стояв упритул
   * до межі й одного разу за неї вийшов.
   *
   * Числа виросли з віялом року в `organicAdapter`: більше атракторів
   * розв'язується, отже більше вузлів скелета й більше роботи по сітці. До
   * зміни ті самі виміри давали 93 / 108 / 129 / 145 мс — тобто дороге тут
   * десятиріччя, +35%. Трикутники при цьому не зросли (16 661 -> 16 904 на
   * десятому році при стелі 18 000), і жоден контракт не порушено.
   */
  60_000);
