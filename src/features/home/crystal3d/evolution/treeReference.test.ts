...
describe('наше дерево проти еталона', () => {
  ...
  it('форма не розходиться з еталоном більше, ніж уже розходиться', () => {
    /*
     * Виміряно: 0.358 -> 0.141 (ADR-0105) -> 0.106 (ADR-0106) -> 0.149
     * (ADR-0107). Числа від 0.141 — проти УТОЧНЕНОГО еталона: його крону
     * перебудовано з дискретних куль на оболонку обертання, бо кулі давали
     * пилчасту огинальну з розмахом 30% у верхній чверті.
     *
     * ПОРІГ ПІДНЯТО, І ЦЕ ТРЕБА НАЗВАТИ ПРЯМО.
     *
     * 0.106 виміряно на дереві, у якого на сороковому році СИМУЛЬОВАНОЇ
     * КРОНИ НЕ БУЛО ВЗАГАЛІ: обрізання лишало одну гілку з шістдесяти двох
     * (ADR-0107 §1), і весь силует малювали сім доданих скелетних гілок,
     * чиє звуження догори підбиралось виміром проти цього ж еталона. Тобто
     * 0.106 — це число ПІДІГНАНОЇ СЕМИПРОМЕНЕВОЇ ЗІРКИ, а не дерева.
     *
     * Тепер крона на сороковому році має п'ятнадцять гілок симуляції, і всі
     * вони — разом зі скелетними — сидять під однією огинальною
     * (`crownProfile.ts`). Що лишилось у розриві, названо: листя за цю
     * огинальну виходить, бо його розкладає окремий контракт
     * (`crownSilhouette`), і саме воно тримає нижні смуги на 0.31-0.41 при
     * еталонних 0.14-0.34.
     *
     * Три віки міряються тепер разом і дають 0.151 / 0.152 / 0.149 — розрив
     * перестав залежати від віку, і це нове.
     *
     * ADR-0108 закрив те, що ADR-0107 назвав як лишене: листя за огинальну
     * більше не виходить. 0.141 / 0.121 / **0.117**, і поріг стягнуто назад
     * до 0.13 — храповик мусить стерегти здобуте, а не терпіти вже
     * полагоджене.
     *
     * ADR-0109 закрив три речі одразу — листя в чистому стовбурі, стелю,
     * зміряну від центра коробки замість стовбура, і півширину, яка тепер
     * дорівнює оголошеній еталоном: 0.103 / 0.114 / **0.113**.
     */
    expect(treeProfileDistance(reference, mature)).toBeLessThan(0.125);
  });
  ...
  it('чистий стовбур не задирається', () => {
     ...
     */
    /*
     * Після ADR-0109 — 0.225, і це вже не «крона починається зависоко», а
     * майже еталон (0.275). Причина була не в тому, ДЕ починається крона, а
     * в тому, що листя росло НИЖЧЕ за неї: у смузі 0.20-0.25 зросту гілки
     * давали 0.097 півширини, а листя 0.200. Тепер згусток не сідає нижче
     * за низ крони (`treeFoliage.ts`).
     *
     * Поріг лишає одну смугу запасу: сама мірка квантована двадцятьма
     * смугами, тож наступний крок угору — це 0.275.
     */
    expect(mature.clearBole).toBeLessThan(0.28);
  });

  it('молоде дерево теж не ширше за еталон', () => {
    /*
     * ЦЕ НОВЕ, І ЦЕ ЗАКРИТИЙ ПУНКТ ADR-0106 §7.
     *
     * Ширину на дванадцяти й двадцяти роках задавала САМА СИМУЛЯЦІЯ, і вона
     * давала 0.516 та 0.481 при еталонних 0.423 — важіль скелетних гілок до
     * неї не дотягувався взагалі. Тепер симуляція йде під ту саму
     * огинальну: 0.451 і 0.427.
     */
    expect(ourProfile(12).spread).toBeLessThan(0.48);
    expect(ourProfile(20).spread).toBeLessThan(0.46);
  });
  ...
  it('крона не ширшає далі', () => {
     ...
     */
    expect(mature.spread).toBeLessThan(0.40);
  });
  ...
  it('верхівка не стає ще пласкішою', () => {
     ...
     * 0.423, тобто 11%.
     *
     * Після ADR-0107 — 0.125 при 0.405, тобто 31%, і поріг піднято з тієї ж
     * причини, що й вище: 0.093 міряло верхівку, якої не існувало. Крони
     * симуляції на сороковому році не було, найвища скелетна гілка сходила
     * за підігнаним лінійним законом, і виходило вістря. Тепер угорі є
     * справжня крона, обмежена огинальною, а понад неї виходить листя.
     *
     * Для порівняння: без огинальної, але з полагодженим обрізанням, ця
     * смуга міряла 0.373 при 0.438 — 85%, тобто зріз.
     *
     * Після ADR-0108 — 0.107 при 0.395, тобто 27%, і поріг стягнуто назад
     * на 0.30.
     *
     * Після ADR-0109 — 0.012 при 0.371, тобто 3%. Тепер верхівка сходить
     * НАДТО різко: в еталона 11%. Це записано як межа, а не як здобуток, і
     * поріг лишається односторонім навмисно — він стереже від ЗРІЗУ, а
     * надто гостра верхівка це інша вада, і ловити її цим числом означало б
     * зробити з храповика два пороги, між якими нема місця.
     */
