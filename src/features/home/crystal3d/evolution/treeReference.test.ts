...
describe('наше дерево проти еталона', () => {
  ...
  it('форма не розходиться з еталоном більше, ніж уже розходиться', () => {
     ...
     *
     * ADR-0111 опустив найвищу скелетну гілку з 0.88 на 0.72 зросту й дав
     * гілкам донести оголошений виліт: 0.101 / 0.102 / **0.107**.
     *
     * МЕЖА САМОЇ МІРКИ, НАЗВАНА ТУТ, БО ЦЕ ВАЖЛИВІШЕ ЗА ЧИСЛО. Відстань
     * профілів міряє МАКСИМУМ радіуса в кожній смузі — тобто огинальну, і
     * тільки її. Вона не бачить ні порожнечі всередині крони, ні талії:
     * смуга завширшки 43 пікселі між сусідніми 182 і 65 дає той самий
     * внесок, що й повна. ADR-0111 двічі виміряв «покращення», яке на
     * екрані було діркою в кроні. Числу тут вірити можна рівно доти, доки
     * поруч є знімок.
     */
    expect(treeProfileDistance(reference, mature)).toBeLessThan(0.115);
  });
