...
describe('кристал від 0 до 40 років', () => {
  for (const profile of Object.keys(PROFILES) as Profile[]) {
    describe(profile, () => {
      ...
      it('щороку вищий за себе торішнього', () => {
         ...
         *
         * ПРИЧИНУ ДОВЕДЕНО МУТАЦІЄЮ, А ЛІКУВАННЯ ВІДХИЛЕНО ВИМІРОМ
         * (ADR-0127). З `APEX_CUT = 0` висота на 30, 33, 36 і 40 роках
         * однакова до п'ятого знака (1.49730) — тобто зріз вістря це сто
         * відсотків втрати, а не одна з причин.
         *
         * Три спроби зняти податок виміряні й відкинуті. Найкраща —
         * піднімати саму лише корону — робить дрейф +0.017% замість
         * −0.154%, тобто майже рівно; але коштує вона +1.5% висоти на
         * КОЖНОМУ віці, і кристал стає стрункішим (3.956 → 4.024), тоді
         * як еталон кремезніший. Відстань до еталона росте на 5.4% на
         * першому році й на 10.5% на сороковому, і засувка 0.17 падає.
         *
         * Тобто зріз — не вада проти еталона: наше налаштування його вже
         * враховує. Прибрати дрейф усередині тому геометрії не можна
         * інакше, як зробивши податок функцією ІСТОРІЇ тіла, а історії
         * геометрія не знає й знати не має.
         *
         * Допуск ЗАТИСНУТО з 0.25% до 0.2%: він тепер стереже виміряні
         * 0.154%, а не вдвічі більший запас «про всяк випадок».
         *
         * **0.2% → 0.25% (ADR-0150), і причина та сама, що й раніше.**
         * Монарх став кремезнішим на вказівку власника (номінальна
         * стрункість 5.4 → 4.0), тобто тіло ширше на третину. Зріз
         * вістря — ЧАСТКА РАДІУСА, тож податок виріс разом із радіусом,
         * і виміряний дрейф пішов за ним рівно пропорційно:
         *
         *   порожня 0.075%   лише фото 0.060%   середня 0.110%
         *   лабораторна 0.000%   активна 0.208%  ← найгірше
         *
         * 0.208% від 1.4746 — це 0.0031 одиниці, тобто два пікселі на
         * кристалі заввишки близько 900. Механізм не новий і не
         * прихований: він названий вище, виміряний тоді й переміряний
         * тепер.
         */
        let previous = 0;
        for (const years of AGES) {
      ...
      it('тримає паралельні боки на кожному віці', () => {
        for (const years of AGES) {
          ...
          /*
           * СТЕЛЯ 1.15 → 1.18 (ADR-0158), і це вимір мірки, а не тіла.
           *
           * `flare` — відношення смуги профілю біля плеча до смуги біля
           * підошви, а смуга бере НАЙДАЛЬШУ від осі точку, тобто описане
           * коло. При дев'яти сторонах (непарне число) кут біля підошви й
           * кут біля плеча стоять під різними азимутами, і різниця між
           * гранню та кутом — 1/cos(π/9) = 1.064 — заходить у відношення
           * нерівно. Виміряно 1.1561 на порожньому профілі; боки при
           * цьому паралельні, що стереже сам еталонний храповик
           * (`ПРИЗМА СТАЛА ПРИЗМОЮ`, смуга 0.9–1.12 на приведеному
           * профілі).
           */
          expect({ years, flare: flare > 0.85 && flare < 1.18 })
      ...
      it('має породу навколо себе на кожному віці', () => {
        for (const years of AGES) {
          ...
          /*
           * Виміряно на всій сітці: найнижче 0.189 на нульовому році,
           * найвище 0.300 на третьому. Порода тримається близько п'ятої
           * частини.
           *
           * Поріг 0.19 → 0.185 (ADR-0154): тупий габітус доростав лише до
           * 93% оголошеної висоти, і коли зріз верхівки зняли на вказівку
           * власника, тіло стало на 7% вищим при тій самій породі. Це не
           * порода осіла, а кристал доріс — числом 0.196 → 0.189.
           */
          expect({ years, low: rockRise < 0.185 }).toEqual({ years, low: false });
        }
      }, 300_000);
  ...
  /*
   * ПЕРШИЙ РІЧНИЙ КРИСТАЛ ЗАХОДИВ У МОНАРХА — вада, яку знайшла ця
   * розгортка, і яку ADR-0125 закрив.
   *
   * `colonyClearance.test.ts` міряв те саме на власній фікстурі з подій і
   * проходив. Тут інший зріз — заповнення пісочниці, — і на ньому
   * `crystal:year:1` стояв усередині монарха з нульового до п'ятого року
   * на КОЖНОМУ профілі: −0.0011 на нульовому, −0.0071 на третьому,
   * тобто 1.4% висоти тіла.
   *
   * Причина: оголошений радіус не був півшириною тіла — монарх
   * малювався в 1.51 раза ширшим за число, за яким його обходять. Тепер
   * посадка бере виміряну півширину підошви на габітус
   * (`MONARCH_FOOT_WIDTH` / `CHILD_FOOT_WIDTH`), і перетину немає в
   * жодному віці на жодному профілі.
   *
   * Тому засувка стоїть на нулі, а не на −1.6%, як тимчасова межа перед
   * тим. Вимір після правки: найтісніше місце ніде не сідає нижче нуля.
   */
  it('жодна дитина не заходить у монарха в жодному віці (ADR-0125)', () => {
    for (const years of [0, 1, 3, 5, 8, 20, 40]) {
      for (const profile of Object.keys(PROFILES) as Profile[]) {
        const { monarch, meshes } = shoot(years, profile);
