...
const PROFILES = {
  ...
  /*
   * СЕРЕДИНА ДОДАНА ПІСЛЯ ТОГО, ЯК КРІЗЬ НЕЇ ПРОЙШЛА ВАДА (ADR-0113).
   *
   * Тут стояли три профілі — порожня, лише фото, активна, — і всі три
   * тримали бюджет. А мобільну стелю трикутників вибивали саме СЕРЕДНІ
   * пари: «середня» на двадцять сьомому році давала 18 262 при 18 000, а
   * «лабораторна» — 18 761 на двадцять шостому. Тобто три профілі покривали
   * краї й лишали середину, де живе більшість пар.
   *
   * Обидва нові беруться з `treeLab.tsx` — тими самими числами, якими
   * знімається лабораторія, щоб знімок і тест говорили про одне дерево.
   */
 ...
 */
const AGES = [
  0, 0.25, 0.5, 1, 2, 3, 5, 8, 12, 17, 18, 20, 21, 22, 26, 27, 30, 40,
] as const;
/*
 * Двадцять шостий і двадцять сьомий стоять тут із тієї ж причини, що й
 * решта: саме на них виміряно перебір бюджету (ADR-0113). Сітка й далі
 * складена з місць, де вже ламалось.
 */
