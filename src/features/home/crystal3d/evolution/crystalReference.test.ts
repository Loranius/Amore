...
describe('насип навколо кристалів існує', () => {
  /*
   * РЕГРЕСІЯ, І ВОНА ПРО МЕРТВУ ФУНКЦІЮ (ADR-0156).
   *
   * Власник, дивлячись на портал: «камінці, які оточують кристали, вони
   * без текстури верху, без кришки». Виміряно — камінців не було зовсім:
   * з двадцяти двох оголошених брил малювалось НУЛЬ на 1, 2, 4, 8 і 11
   * роках, три на 25 і шістнадцять на 40. Те, що він бачив, було самим
   * коміром — тілом обертання, чиї грані за побудовою радіальні клини.
   *
   * ЧОМУ ЦЕ НЕ ЗЛОВИВ ТЕСТ, ЯКИЙ ДЛЯ ЦЬОГО Й НАПИСАНИЙ. `БРИЛИ Є`
   * (`geometry/sharedRoot.test.ts`) перевіряє те саме — і проходив. Він
   * будує СВОЮ колонію з іншим набором подій, і в ній брили були. Тобто
   * гарантія трималась на синтетичній парі, а пара з порталу лишалась
   * без каміння.
   *
   * Тому мірка тут — той самий ланцюг станів, яким живе лабораторія й
   * портал (`buildCrystalPipelineStates`), і ті самі числа заповнення.
   */
  it('на кожному віці, який пара реально проживе', () => {
    for (const years of [1, 2, 4, 11, 25, 40]) {
      const states = statesOf(years);
      const substrate = states.geometry.meshes.find(
        (mesh) => mesh.bodyId === CRYSTAL_SUBSTRATE_BODY_ID,
      );
      expect(substrate, `${years}р`).toBeDefined();
      const seam = substrate!.profile.seamTriangleCount!;
      const stones = substrate!.indices.length / 3 - seam;
      /*
       * Не «більше нуля»: одна брила на всю купу — це той самий нуль із
       * точністю до випадковості. Виміряно 484…572 трикутники каменю на
       * цих віках, тобто близько двадцяти чотирьох каменів.
       */
      expect(stones, `${years}р трикутників каменю`).toBeGreaterThan(200);
    }
  });
});
