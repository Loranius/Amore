...
/**
 * На якій частці власної висоти промінь гасне — НАД дном, не під ним.
 *
 * 0.08 від `top` — це близько чверті радіуса каменя, тобто вище за будь-яку
 * дюну (`DUNE_HEIGHT` дає десяті частки) і достатньо низько, щоб промінь
 * ще читався таким, що дістає майже до дна.
 */
const BOTTOM_STOP = 0.08;
  ...
  for (let shaft = 0; shaft < SHAFT_COUNT; shaft += 1) {
    ...
    for (const plane of [0, 1]) {
      ...
      for (let row = 0; row < 2; row += 1) {
        /*
         * НИЖНІЙ РЯД СТОЇТЬ НАД ДНОМ, А НЕ ПІД НИМ (ADR-0197).
         *
         * Тут було `-top * 0.2`: площина променя прорізала пісок, і
         * видима її частина обривалась по ЛІНІЇ ПЕРЕТИНУ — блідий
         * прямокутник із прямими краями лежав на дні поруч із рифом. Його
         * видно на кадрах від першого дня цієї роботи, і тричі його
         * приписували іншому: наметові піску, серпанку дна, тіньовій
         * камері. Бісекція (сховати шар) назвала винного з першої спроби.
         *
         * Заголовок цього ж файлу при цьому каже, що промені «тануть, не
         * діставши дна». Намір описано, дії не було.
         */
        const height = row === 0 ? top : top * BOTTOM_STOP;
