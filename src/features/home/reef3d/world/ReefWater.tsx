...
/**
 * Скільки радіусів каменя обіймає тіньова камера.
 *
 * ЦЕ КОМПРОМІС МІЖ ДВОМА ВИМІРАМИ, І ОБИДВА ЗНЯТІ ЗІ ЗНІМКА.
 *
 * Вузька рамка дає різкі тіні, але її МЕЖА ВИДНА: за нею дно перестає
 * приймати тінь, і на піску лягає пряма лінія — рівно той гострий кут,
 * проти якого зроблено ADR-0195. При двох радіусах вона падала на чистий
 * пісок і читалась одразу.
 *
 * Широка рамка межі не має, але 1024 пікселі карти розмазуються: при
 * чотирьох радіусах тексель — 5.4 пікселя екрана, при восьми вже
 * одинадцять, і тінь корала стає плямою.
 *
 * Чотири — точка, де межа падає туди, де серпанок дна вже наполовину
 * з'їв пісок (згасання йде від 2.1 до 7.2 радіуса), а тінь ще тримає
 * форму.
 */
  ...
  return (
    <>
      ...
      {/*
        ...
        * лягають на голову, інакше рельєф зникає.
        *
        * `castShadow` тут З'ЯВИВСЯ ЛИШЕ В ADR-0197. Доти цей коментар
        * обіцяв тіні, яких не було: прапорці `castShadow` на голові й
        * колоніях не роблять нічого, поки жодне світло їх не кидає. Рамка
        * тіньової камери щільно обіймає риф — при ширшій 1024 пікселі
        * карти розмазуються по воді, де тіні нема кому ловити.
        */}
      <directionalLight
        ...
        shadow-bias={-0.0015}
        shadow-camera-left={-sceneRadius * SHADOW_REACH}
        shadow-camera-right={sceneRadius * SHADOW_REACH}
        shadow-camera-top={sceneRadius * SHADOW_REACH}
        shadow-camera-bottom={-sceneRadius * SHADOW_REACH}
        shadow-camera-near={sceneRadius * 0.5}
