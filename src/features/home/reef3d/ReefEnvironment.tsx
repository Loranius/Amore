...
const TERRACE_STONES: readonly RockMassProps[] = [
  { position: [-3.65, -0.25, 3.2], scale: [1.25, 0.3, 0.95], rotation: [0, 0.12, 0.05], color: '#596f68' },
  { position: [-2.95, -0.27, 4.05], scale: [1.05, 0.25, 0.82], rotation: [0.03, -0.32, -0.03], color: '#5f756c' },
  { position: [3.55, -0.26, 3.35], scale: [1.32, 0.3, 0.96], rotation: [0.02, -0.18, -0.04], color: '#596f68' },
  { position: [2.8, -0.28, 4.2], scale: [0.98, 0.24, 0.78], rotation: [-0.02, 0.38, 0.04], color: '#60776d' },
  { position: [-3.4, -0.31, -4.2], scale: [1.4, 0.34, 1], rotation: [0, 0.36, 0], color: '#4b6864' },
  { position: [3.45, -0.31, -4.35], scale: [1.46, 0.36, 1.05], rotation: [0, -0.3, 0], color: '#496661' },
];
...
function SandPatch({
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial color="#a7a18a" roughness={1} metalness={0} />
    </mesh>
  );
}
...
function DistantSpire({
  ...
  return (
    <group
      ...
      scale={scale}
    >
      <RockMass position={[0, 0, 0]} scale={[1.55, 0.72, 1.35]} color="#31585a" />
      <RockMass position={[-0.18, 1.02, 0.04]} scale={[0.88, 1.12, 0.82]} rotation={[0.05, 0.24, -0.08]} color="#2d5357" />
      <RockMass position={[0.14, 2.05, -0.03]} scale={[0.58, 0.86, 0.56]} rotation={[-0.04, -0.2, 0.07]} color="#294e54" />
    </group>
  );
}

/**
 * Stage 1 reef environment.
 *
 * This layer intentionally owns only static terrain and distant silhouettes.
 * The generated reef remains a separate production object, while caustics,
 * particles, vegetation and animal life can be layered in later passes.
 */
export function ReefEnvironment() {
  return (
    <group name="reef-environment-stage-1">
      ...
      <mesh position={[0, -0.36, 0]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow={false}>
        ...
        <meshStandardMaterial color="#777c6d" roughness={1} metalness={0} />
      </mesh>
      ...
      <mesh position={[0, -0.39, 0]} scale={[4.25, 0.18, 3.65]} receiveShadow={false}>
        ...
        <meshStandardMaterial color="#6e7568" roughness={1} metalness={0} />
      </mesh>
