...
export interface EvolutionHistoryCounts {
  ...
  /**
   * Скільки фото існувало на цей вік. Значення надійне лише тоді, коли фото
   * прийшли З ДАТАМИ (`photosDated`); інакше це агрегат «сьогодні», видимий
   * на будь-який вік.
   */
  photos: number;
  /**
   * З них — ті, чий вік справді відомий. Рівно це число мають право читати
   * правила форми (`species/crystalConstraints.ts`): недатований агрегат
   * зрушив би всю вже відкладену масу й зламав append-only, тож для нього
   * тут лишається 0.
   */
  photosDated: number;
  /** Агрегат «сьогодні» (власної історії не має). */
  totalSaved: number;
}
