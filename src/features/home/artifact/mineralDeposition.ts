...
/** Глибина поховання основи, у власних радіусах нового тіла. */
const BURIAL = 0.45;
/** Мінімальна вертикальна складова напрямку — друза росте вгору-назовні. */
const MIN_UPWARD = 0.14;
...
/** Успадкування напрямку: нормаль поверхні + вісь субстрату + мале збурення. */
function inheritDirection(normal: Vec3, substrateDirection: Vec3, rng: () => number): Vec3 {
  const dir = normalize(
    add(add(scale(normal, 0.55), scale(substrateDirection, 0.3)), scale(perturbVec(rng), 0.15)),
  );
  if (dir.y >= MIN_UPWARD) return dir;
  // М'яке підняття до мінімальної вертикалі — без стрибка напрямку.
  return normalize(v3(dir.x, MIN_UPWARD + (dir.y < 0 ? 0.08 : 0), dir.z));
}

/** «Гравітаційна компакція»: що вище нуклеювало тіло, то стриманіший ріст —
 *  маса лишається тісною брилою, а не вежею. */
const heightDamp = (anchorY: number): number => 1 / (1 + 0.35 * Math.max(0, anchorY + 0.1));

/** Занурена в поверхню тіла основа: скелетна і rendered водночас. */
function buriedAnchors(body: BodyPair, t: number, angle: number, radius: number, maturity: number) {
  ...
  return {
    skeletalSample,
    anchor: add(skeletalSample.point, scale(skeletalSample.normal, -radius * BURIAL)),
    renderedAnchor: add(
      renderedSample.point,
      scale(renderedSample.normal, -radius * MATURITY_RADIUS_SCALE(maturity) * BURIAL),
    ),
  };
}
 ...
 * 5 (розміри/оберт/дихання) → K×3 (кандидати) → 1 (рулетка) → 2 (напрямок)
 * → 2 (колонія: шанс, розмір). Супутники читають ВЛАСНІ потоки реєстру.
 */
function depositMineral(
  ...
  const rawLength = event.lengthMin + rng() * event.lengthRange;
  const rawRadius = event.radiusMin + rng() * event.radiusRange;
  ...
  const length = rawLength * (0.55 + 0.45 * energy) * heightDamp(probe.point.y);
  const radius = rawRadius * (0.7 + 0.3 * energy);

  // Успадкування напрямку (2 draw).
  const direction = inheritDirection(probe.normal, host.direction, rng);
  ...
  if (roll >= Math.min(0.55, COLONY_CHANCE[event.kind] + shareBoost)) return [dominant];

  // Дрібні тіла нуклеюють максимум пару супутників — не перф-вибух.
  const maxCount = rawRadius < 0.08 ? 2 : 5;
  ...
  for (let s = 0; s < 5; s++) {
    ...
    const dt = (srng() - 0.5) * 0.16;
    const dAngle = (srng() - 0.5) * 0.9;
    const lenFactor = 0.28 + srng() * 0.3;
    const radFactor = 0.4 + srng() * 0.3;
    ...
    // друза): той самий хост, легкий зсув по t/куту, віяло напрямків
    // навколо осі домінанта — родина, а не гілки.
    const satDirection = normalize(add(scale(direction, 0.8), scale(dirJitter, 0.25)));
...
export function depositMineralMass(input: ArtifactInput, pressures: EvolutionPressures): ArtifactNode[] {
  ...
  for (const stream of streams) {
    ...
    for (const event of stream.events) {
      ...
      const colony = depositMineral(event, substrate, seedNum, dna, field);
