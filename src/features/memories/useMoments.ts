 ...
 *
 * `visitedAt` — дата самого спогаду, і це ВІДНОВЛЕННЯ правила, а не нове
 * рішення: на робочій базі в кожної мітки, прив'язаної до спогаду,
 * `visited_at` збігається з датою того спогаду день у день. Правило
 * зникло з коду, і перша ж мітка, створена після цього, лишилась без
 * дати — а мітка без `visited_at` невидима рушієві (`adapters/map.ts`).
 *
 * Цей шлях — ЄДИНИЙ живий спосіб створити мітку в порталі: другий, у
 * `useMapPinMutations`, не має жодного споживача. Тож без цього рядка
 * карта — один із шести модулів року — переставала б заробляти з кожною
 * новою міткою.
 */
