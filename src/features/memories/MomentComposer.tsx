    ...
    <div className="mm-composer" role="dialog" aria-modal="true" aria-label="Спогад">
      ...
      <div className="mm-comp-body">
        <input
          ...
          enterKeyHint="next"
        />
        {/* Нотатка — БАГАТОРЯДКОВА й займає всю висоту, що лишилась.
            Була однорядковим `<input>` з межею у 30 символів, і межа стояла
            в самій базі (`memory_moments_note_len`), тож ніяка розкладка не
            зробила б із неї блокнот. Тепер 2000 — стільки ж, скільки вже
            дозволено опису фото. */}
        <textarea
          className="mm-note-input"
          value={note}
          onChange={(e) => setNote(e.target.value.slice(0, NOTE_LIMIT))}
          placeholder="Що сталося того дня…"
          maxLength={NOTE_LIMIT}
          rows={1}
        />

        {/* Дата, місце й підказка стоять ВНИЗУ, під текстом.
            ------------------------------------------------------------
            Прохання власника, і воно правильне: у телефонному блокноті
            зверху лежить те, що пишеш, а службове — знизу. Раніше дата
            з підказкою займали місце одразу під назвою, тобто перше, що
            бачила пара, було не полем для запису, а двома рядками про
            те, як цим користуватись. */}
        <div className="mm-comp-foot">
        ...
        {photoCount === 0 && (
          <p className="mm-comp-hint">
            Натисни скріпку — і додай фото, місце чи дату.
          </p>
        )}
        </div>
      </div>
