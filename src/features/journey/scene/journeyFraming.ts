...
export interface JourneySplit {
  /** Наскільки зсунути ПРИЦІЛ уздовж екранного «праворуч», одиниці сцени. */
  right: number;
  /** Наскільки зсунути приціл уздовж екранного «вгору», одиниці сцени. */
  up: number;
}

/**
 * Куди відвести приціл, щоб подія пішла геть від панелі деталей.
 *
 * Живе тут чистою функцією саме через знак. Камера дивиться на ПРИЦІЛ, тобто
 * приціл лягає в центр кадру, а подія опиняється з протилежного боку від
 * зсуву: приціл нижче — подія вище, приціл праворуч — подія ліворуч. Одну з
 * двох гілок це збивало, і сонце їхало ПІД панель замість того, щоб від неї
 * відсунутись. Побачити це стало можливо лише тоді, коли сонце перестало
 * заповнювати кадр цілком, — тому тепер знак стереже тест, а не око.
 */
export function journeySplit(viewport: JourneyViewport, distance: number): JourneySplit {
  const halfHeight = distance * Math.tan((viewport.fovY * Math.PI) / 360);
  // Деталі знизу на вузькому екрані й праворуч на широкому.
  if (viewport.aspect < 1) return { right: 0, up: -SPLIT_SHIFT * halfHeight };
  return { right: SPLIT_SHIFT * halfHeight * Math.max(0.05, viewport.aspect), up: 0 };
}
