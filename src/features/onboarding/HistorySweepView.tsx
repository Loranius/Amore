...
export function HistorySweepView({
  ...
  return (
    <div className="page sweep-page">
      ...
      {/*
        * ВИБІР ВИДУ ПОКАЗУЄТЬСЯ ВІД ТОГО МОМЕНТУ, ЯК Є ДАТА, А НЕ ЛИШЕ НА
        * КРОЦІ РОКІВ (ADR-0209 §13).
        *
        * ВАДА, ЗА ФАКТОМ ЯКОЇ ЗМІНЕНО, І ЗНАЙШЛАСЬ ВОНА ЛИШЕ ТОМУ, ЩО
        * ЗʼЯВИЛАСЬ РЕЄСТРАЦІЯ. Крок виводиться так: є дата й нуль річних
        * річниць → `anniversaries`. Щойно зареєстрована пара має рівно
        * це. А `SweepSpecies` і «До артефакта» стояли ТІЛЬКИ під
        * `step === 'years'` — тобто пара, яка щойно створила портал, не
        * могла ні обрати вид, ні завершити прохід, доки не додасть
        * річницю. Вибір виду, який власник просив у реєстрації, був для
        * нової пари недосяжним.
        *
        * Умова власника при цьому не порушена: вид вибирають «на своїй
        * історії, а не на порожньому екрані з трьома картинками»
        * (`SweepSpecies.tsx`). Історію тут дає ДАТА — `summary.years`
        * рахується з неї й не залежить від річниць зовсім
        * (`useHistorySweep.ts`), тож сцена позаду вже показує справжній
        * артефакт пари з їхніми роками. Тому умова — `years.length > 0`,
        * а не назва кроку.
        */}
      {years.length > 0 && <SweepSpecies yearCount={years.length} />}
