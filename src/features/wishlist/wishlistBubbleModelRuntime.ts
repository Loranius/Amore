...
const FALLBACK_CLASS = 'wl-cloud-bubble--model-unavailable';
...
function priorityOpacity(element: HTMLElement): number {
  if (element.dataset.priority === 'high') return 0.82;
  if (element.dataset.priority === 'low') return 0.58;
  return 0.7;
}
...
function normalizeModel(root: Group): Group {
  ...
  const scale = 1.94 / largestDimension;
...
function mountRenderer(element: HTMLElement, sourceModel: Group): () => void {
  ...
  try {
    ...
    console.info('[Wishlist] WebGL is unavailable; showing the gift without the GLB shell.', error);
    return () => element.classList.remove(FALLBACK_CLASS);
  }
