...
import { MAX_PIXEL_RATIO, maxPixelRatio, stepFor, thumbUrl } from './imageCdn';
import { jpegEquivalent } from './images';
...
describe('адреса мініатюри', () => {
  ...
  it('ДРІБНИЙ КАДР ДІСТАЄ ПОВНУ ЩІЛЬНІСТЬ, великий — обрізану', () => {
    /*
     * Тут стояло `MAX_PIXEL_RATIO === 2` з обґрунтуванням «різницю на
     * фотографії побачити майже неможливо». Це було твердження, а не
     * вимір, і вимір його не підтвердив: акутанс кадру, зменшеного до
     * DPR 2 і повернутого назад до DPR 3, — **83% від чесного** на 128
     * CSS px, 85% на 192, 86% на 256. Тобто на айфоні й щільному андроїді
     * пара бачила фотографії на шосту частину м'якшими, ніж на віндовсі.
     *
     * Дрібні кадри тепер дістають повну щільність: за виміряною таблицею
     * ваги крок 256 → 384 коштує близько 32 КБ на знімок, і саме на
     * дрібних кадрах різкість помітна найбільше. Великі лишаються на
     * двійці — там зайвий крок коштує вже сотні кілобайт, а DPR 2 і так
     * дає 768–1600 пікселів.
     */
    expect(maxPixelRatio(128)).toBe(3);
    expect(maxPixelRatio(256)).toBe(3);
    expect(maxPixelRatio(257)).toBe(2);
    expect(maxPixelRatio(1080)).toBe(2);
    expect(MAX_PIXEL_RATIO).toBe(3);
  });

  it('дрібний кадр на DPR 3 просить ширину, а не м’якість', () => {
    expect(thumbUrl(OBJECT, 128, { dpr: 3 })).toContain('width=384');
    // А великий — ні: там стеля лишилась двійкою, тож ширина та сама, що
    // й на DPR 2, і зайвих сотень кілобайт не з'являється.
    expect(thumbUrl(OBJECT, 512, { dpr: 2 })).toContain('width=1080');
  });
...
describe('якість майстер-файла не залежить від телефона', () => {
  it('JPEG дістає свій відповідник заданої якості WebP', () => {
    /*
     * `compress` віддає WebP там, де полотно вміє його кодувати, і JPEG
     * там, де не вміє — тобто формат майстер-файла вирішує ТЕЛЕФОН, з
     * якого пара додала знімок. Поки число було спільним, 0.78 означало
     * помітно різну картинку: JPEG на 0.78 дає блоки на градієнтах, а
     * WebP на 0.78 — ні. Той самий кадр зберігався по-різному, і різницю
     * було видно на всіх платформах, бо майстер-файл один на всіх.
     *
     * Число не виміряне (чужий кодувальник у пісочниці міряти нічим) і
     * записане як припущення в `images.ts`.
     */
    expect(jpegEquivalent(0.78)).toBeCloseTo(0.88, 5);
    expect(jpegEquivalent(0.84)).toBeCloseTo(0.94, 5);
    // Стеля: вище 0.95 JPEG росте вагою, а не якістю.
    expect(jpegEquivalent(0.95)).toBe(0.95);
    // Сміття на вході не має ставати сміттям на виході.
    expect(jpegEquivalent(Number.NaN)).toBeCloseTo(0.88, 5);
  });
});
