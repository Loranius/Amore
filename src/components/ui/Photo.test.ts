...
describe('<Photo> — рятунок завеликого оригіналу', () => {
  ...
  it('просить ОДНУ сторону: дві вбивають пропорції', () => {
    /*
     * ЗМІНЕНЕ ОЧІКУВАННЯ, І ОСЬ ЧОМУ (ADR-0203).
     *
     * Тут стояло `expect(PHOTO).toContain('resizeHeight: targetPx')` —
     * тобто тест вимагав рівно ту ваду, яку мав би ловити. Дві однакові
     * сторони в `createImageBitmap` означають не «вписати в квадрат», а
     * «стиснути рівно в ці числа».
     *
     * ВИМІРЯНО в справжньому Chromium на знімку 200×100:
     *
     *   обидві сторони по 64   → 64×64   — пропорції гинуть
     *   сама лише ширина 64    → 64×32   — пропорції цілі
     *
     * Отже кожне неквадратне фото, яке доходило до рятунку, лягало на
     * екран розплющеним — і саме там, де рятунок потрібен, тобто на
     * великому знімку в повний екран.
     *
     * Довгу сторону доводить до межі полотно, і воно ж цю межу поважає.
     */
