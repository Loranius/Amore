...
const DataCache = (() => {
  ...
  return { get, set, has, invalidate, invalidatePrefix, swr, ensure, fadeRender };
})();

// window.X = X — стандартний спосіб публікувати глобаль у цьому проєкті
// (немає модулів, немає declare global {} без import/export). TS не знає
// про DataCache на Window без module-augmentation, тому вузький, разовий
// каст на window тут — той самий "немає білду" виняток, що й в іншому.
/** @type {any} */ (window).DataCache = DataCache;
