...
import { DailyQuestion } from '../modules/question.js';
import { MapModule } from '../modules/map.js';
import { Media } from '../modules/media.js';
import { PhotoCalendar } from '../modules/photo-calendar.js';
import { RandomModule } from '../modules/random.js';
...
const MAP = {
  events: {
    ...
    onChange() {
      ...
      if (v === 'calendar') CalendarModule.refresh();
      if (v === 'home' || v === 'calendar') Counter.render();
    },
  },
...
export const Realtime = { init, start, stop, markSelf };
...
//
// ВАЖЛИВО (ESM): патчимо САМЕ імпортований `supabase` — той самий об'єкт,
// що його викликають усі модулі через import. У window.supabase лежить
// CDN-фабрика (інший об'єкт), її патчити марно.
// ============================================================
patchSupabaseWrites(supabase);

/** @param {any} client @returns {void} */
function patchSupabaseWrites(client) {
  if (!client || typeof client.from !== 'function') return;
  if (client.__amoreWritePatched) return;
  client.__amoreWritePatched = true;

  const _from = client.from.bind(client);
  client.from = function (/** @type {string} */ table) {
    const qb = _from(table);
    ['insert', 'update', 'upsert', 'delete'].forEach((m) => {
      const orig = qb[m];
      if (typeof orig === 'function') {
        qb[m] = function (/** @type {any[]} */ ...args) {
          try { markSelf(table); } catch (e) {}
