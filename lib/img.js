...
const Img = (() => {
  ...
  async function convertViaLibheif(file) {
    ...
    await new Promise((resolve, reject) => {
      image.display(imageData, (ok) => ok ? resolve() : reject(new Error('не вдалося декодувати кадр HEIC')));
    });
    try { images.forEach(im => { if (typeof im.free === 'function') im.free(); }); } catch (_) {}
  ...
  async function convertViaHeicTo(file) {
    ...
    if (!fn) {
      try {
        ...
        const m = await import('https://cdn.jsdelivr.net/npm/heic-to@1.5.2/+esm');
  ...
  async function normalize(file) {
    ...
    throw new Error((lastErr && lastErr.message) ? lastErr.message : 'формат не підтримується');
  }
