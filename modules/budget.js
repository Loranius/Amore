...
const Budget = (() => {
  ...
  function bindTumbochka() {
    ...
    // З доходу
    const incInp=el('tumbo-income-inp'),incBtn=el('tumbo-income-btn');
    const optBtns=document.querySelectorAll('.tumbo-pct-btn');
    const calcEl=el('tumbo-calc'),panel=el('tumbo-pct-panel');
    ...
    const openPanel=()=>{
      ...
      curPct=15; upd(); panel?.classList.add('visible');
    };
  ...
  function renderFreeLimit() {
    const s=LS(), user=Auth.getCurrentUser();
    const limit=s.freeLimit||0;
    const proposal=s.freeLimitProposal||null;
    ...
    if(proposal&&proposal.proposedBy!==user?.name){
      ...
      if(propConf&&!propConf.dataset.bound){
        ...
        propConf.addEventListener('click',()=>{
      ...
      if(propRej&&!propRej.dataset.bound){
        ...
        propRej.addEventListener('click',()=>{ SS({freeLimitProposal:null}); renderFreeLimit(); });
      }
    } else {
