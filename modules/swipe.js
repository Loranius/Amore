...
const Swipe = (() => {
  ...
  const TMDB_TOKEN = 'eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIxYjI4Y2FjYWFiMmY5MGE4YzJiZDBjMzgzYzYzNmYwMSIsIm5iZiI6MTc3NDU2MzIxMC4wNDksInN1YiI6IjY5YzVhZjhhZjg1YTU5MzgwZjg5YmNkYyIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.O7x_NNEEY2whI36uZUfLoh8OH3KtSnKRYvpABjlZzGg';
  ...
  async function fetchTmdb(endpoint) {
    const res = await fetch(`${TMDB_BASE}${endpoint}&language=uk-UA`, {
  ...
  function attachTouchHandlers(el, card) {
    ...
    el.addEventListener('touchmove', (e) => {
      ...
      el.querySelectorAll('.swipe-hint').forEach(h => (h as HTMLElement).style.opacity = '0');
      if (Math.abs(dy) > Math.abs(dx) && dy < -40) {
        (el.querySelector('.swipe-hint-up') as HTMLElement).style.opacity = '1';
      } else if (dx < -40) {
        (el.querySelector('.swipe-hint-left') as HTMLElement).style.opacity = '1';
      } else if (dx > 40) {
        (el.querySelector('.swipe-hint-right') as HTMLElement).style.opacity = '1';
      }
    }, { passive: true });
  ...
  function bindActionButtons() {
    document.getElementById('swipe-btn-up')?.addEventListener('click', () => {
      const top = document.querySelector('#swipe-stack .swipe-card:last-child') as HTMLElement;
      if (top) {
  ...
  function renderTypeTabs() {
    document.querySelectorAll('.swipe-type-btn').forEach(btn => {
      (btn as HTMLElement).classList.toggle('active', (btn as HTMLElement).dataset.swipeType === activeType);
    });
  }
  ...
  function init() {
    document.querySelectorAll('.swipe-type-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        activeType = (btn as HTMLElement).dataset.swipeType || 'movie';
        refresh();
      });
    });
