...
const Swipe = (() => {
  ...
  async function saveVote(card, direction) {
    ...
    if (votes[0].direction === votes[1].direction) {
      handleMatch(card, direction);
    }
  }
  ...
  function showMatchBanner(card, direction) {
    var label = direction === 'up' ? 'Бачили! 🎉' : 'В планах! 📋';
  ...
  async function initStack() {
    ...
    try {
      ...
      if (!cards.length) {
        stack.innerHTML = '<p class="empty-state">Не вдалось завантажити. Перевір з\'єднання.</p>';
        return;
      }
