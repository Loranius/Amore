...
const Swipe = (() => {
  ...
  async function loadCards() {
    const endpoint = activeType === 'movie'
      ? `/discover/movie?sort_by=popularity.desc&page=${page}&with_original_language=uk|en|ru`
      : `/discover/tv?sort_by=popularity.desc&page=${page}&with_original_language=uk|en|ru`;
    ...
    try {
      ...
      return data.results
        .filter(function(item) {
          ...
          // Фільтруємо назви з ієрогліфами (японська, китайська, корейська, арабська тощо)
  ...
  function attachTouch(el, card) {
    ...
    el.addEventListener('touchmove', function(e) {
      ...
      var dx = e.touches[0].clientX - lStartX;
      var dy = e.touches[0].clientY - lStartY;
    ...
    el.addEventListener('touchend', function(e) {
      if (!localDragging) return;
      localDragging = false;
      var dx = e.changedTouches[0].clientX - lStartX;
      var dy = e.changedTouches[0].clientY - lStartY;
      var THRESHOLD = 80;
      ...
      if (Math.abs(dy) > Math.abs(dx)) {
        if (dy < -THRESHOLD) flyOut(el, 'up', card);
        else if (dy > THRESHOLD) flyOut(el, 'down', card);
        else resetCard(el);
      } else {
        if (dx < -THRESHOLD) flyOut(el, 'left', card);
        else if (dx > THRESHOLD) flyOut(el, 'skip', card);
        else resetCard(el);
      }
    }, { passive: true });
  }
  ...
  async function nextCard() {
    ...
    // Підвантажуємо більше якщо мало карток залишилось
    ...
    // Підтримуємо 3 картки в стеку завжди
    var inStack = stack.querySelectorAll('.swipe-card').length;
    var needed = 3 - inStack;

    for (var i = 0; i < needed; i++) {
      var cardIdx = currentIndex + inStack + i;
      if (cardIdx < cards.length) {
        var el = renderCard(cards[cardIdx]);
        el.style.zIndex = String(i);
        stack.insertBefore(el, stack.firstChild);
      }
    }

    // Оновлюємо z-index верхньої картки
  ...
  async function initStack() {
    ...
    try {
      ...
      // Завантажуємо поки не наберемо 20 нових карток
      var attempts = 0;
      while (cards.length < 20 && attempts < 5) {
        attempts++;
      ...
      // Рендеримо перші 3 картки (знизу → вгору)
      var initial = cards.slice(0, 3).reverse();
      initial.forEach(function(card, i) {
