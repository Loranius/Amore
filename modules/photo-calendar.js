...
const PhotoCalendar = (() => {
  ...
  function bindDayModal(ds, myPhoto) {
    ...
    el('pcal-ov')?.addEventListener('click', e => {
      if (e.target.id === 'pcal-ov') closeDayModal();
    });
    ...
    el('pcal-save-comment')?.addEventListener('click', async () => {
      const comment = el('pcal-comment-inp')?.value.trim() || null;
  ...
  async function doUpload(ds, existingId) {
    ...
    const btn = el('pcal-upload-confirm');
    if (btn) { btn.disabled = true; btn.textContent = 'Завантаження…'; }

    try {
      // Стиснення (Img.compress з lib/img.js)
      let blob = pendingFile, ext = 'jpg', contentType = 'image/jpeg';
      try {
        const out = await Img.compress(pendingFile, 1280, 0.82);
        blob = out.blob; ext = out.ext; contentType = out.contentType;
      } catch (e) { console.warn('compress failed, uploading original:', e); }

      // Фіксований шлях → upsert завжди перезаписує той самий файл у Storage
      const [y, m] = ds.split('-');
      const path = `${y}/${m}/${ds}_${currentUser.id}.${ext}`;
      ...
      const comment   = el('pcal-new-comment')?.value.trim() || null;

      // upsert у БД: uniq(date, user_id) → INSERT або UPDATE
      const { error: dbErr } = await supabase.from('photo_calendar').upsert(
        { date: ds, user_id: currentUser.id, photo_url, comment },
      ...
      alert('Помилка завантаження: ' + (e.message || String(e)));
  ...
  function init() {
    ...
    window.addEventListener('portal:view', e => {
      if (e.detail.view === 'photo-calendar') refresh();
    });
  }
