...
const Wishlist = (() => {
  ...
  function openAddModal() {
    const root=el('modal-root');
    // Будуємо форму через DOM щоб уникнути проблем з innerHTML + listeners
    ...
    overlay.querySelector('#wf-cancel').addEventListener('click',()=>root.innerHTML='');
    overlay.addEventListener('click',e=>{ if(e.target===overlay) root.innerHTML=''; });
    overlay.querySelector('#wf-save').addEventListener('click',async()=>{
      const titleInp = overlay.querySelector('#wf-title');
      const title = titleInp.value.trim();
      if(!title){ titleInp.style.borderColor='var(--danger)'; return; }
      const user=Auth.getCurrentUser();
      if(!user){ alert('Потрібно увійти'); return; }
      const saveBtn=overlay.querySelector('#wf-save');
      if(saveBtn) saveBtn.disabled=true;
      const payload = {
        title,
        link:        (overlay.querySelector('#wf-link').value||'').trim()||null,
        image_url:   (overlay.querySelector('#wf-img').value||'').trim()||null,
        description: (overlay.querySelector('#wf-desc').value||'').trim()||null,
        gift_date:   overlay.querySelector('#wf-date').value||null,
        owner:       user.id,
        reserved:    false,
        reserved_by: null
      };
