...
const Wishlist = (() => {
  ...
  async function renderSizes() {
    ...
    // Гарантуємо що users завантажені
    if(!allUsers.length) allUsers=await loadUsers();
    if(!activeOwnerId && current) activeOwnerId=current.id;

    // Оновлюємо active стан кнопок перемикача
    document.querySelectorAll('.sz-user-btn').forEach(btn=>{
      const idx=+btn.dataset.userIdx;
      btn.classList.toggle('active', allUsers[idx]?.id===activeOwnerId);
    });
  ...
  async function refresh() {
    ...
    const current=Auth.getCurrentUser();
    if(!activeOwnerId && current) activeOwnerId=current.id;
    if(activeTab==='dima'||activeTab==='lena') activeTab='wishes';
    renderTabs();
  ...
  function init() {
    ...
    // Перемикач Діма/Лєна в розмірах (статичний HTML)
    document.querySelectorAll('.sz-user-btn').forEach(btn=>{
      btn.addEventListener('click',()=>{
        document.querySelectorAll('.sz-user-btn').forEach(b=>b.classList.remove('active'));
        btn.classList.add('active');
        const idx=+btn.dataset.userIdx;
        if(allUsers[idx]) { activeOwnerId=allUsers[idx].id; renderSizes(); }
        else loadUsers().then(u=>{ allUsers=u; if(allUsers[idx]){activeOwnerId=allUsers[idx].id; renderSizes();} });
      });
    });
