...
const Wishlist = (() => {
  ...
  function renderGrid(items, isOwnList) {
    ...
    items.forEach(item => {
      ...
      card.innerHTML = `
        ...
        ${isOwnList ? `<button class="delete-btn" data-delete-id="${item.id}" title="Видалити">×</button>` : ''}
      `;
  ...
  async function deleteItem(id) {
    if (!confirm('Видалити це бажання?')) return;

    const { error } = await supabase.from('wishlist_items').delete().eq('id', id);
    if (error) {
      console.error('Помилка видалення бажання:', error);
      alert('Не вдалось видалити бажання');
      return;
    }
    refreshGrid();
  }
