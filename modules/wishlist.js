...
const Wishlist = (() => {
  ...
  async function renderGrid() {
    ...
    const visible=wishFilter==='all' ? allItems : allItems.filter(i=>i._ownerId===wishFilter);
    if(!visible.length){
    ...
    // Групуємо по юзерах
    const usersToShow = wishFilter==='all' ? allUsers : allUsers.filter(u=>u.id===wishFilter);
  ...
  function openAddModal() {
    ...
    el('wf-save')?.addEventListener('click',async()=>{
      ...
      const {error} = await supabase.from('wishlist_items').insert({
        ...
        owner:      user.id, reserved:false, reserved_by:null
      });
      if(error){ alert('Помилка: '+error.message); if(saveBtn) saveBtn.disabled=false; return; }
      root.innerHTML='';
      wishFilter=user.id;
      renderGrid();
    });
