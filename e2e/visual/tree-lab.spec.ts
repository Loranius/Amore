...
/*
 * СТЕЛІ ЗГУСТКІВ І ЛИСТЯ ЙДУТЬ ЗА `engine/foliage/config.ts`, А НЕ ЗА ПАМ'ЯТТЮ.
 *
 * Тут стояло 64 згустки й 900 листків — обидва числа застаріли, і обидва
 * рушій уже переглянув із виміром на руках:
 *
 *   • 64 → 220: «64 не мали жодного стосунку до вартості: згусток — це
 *     запис розміщення, трикутники коштує ЛИСТЯ». На живому дереві пари
 *     стеля в 64 відрізала 97 кандидатів зі 140, і 32 гілки з 50 лишались
 *     голими;
 *   • 900 → 700 (ADR-0072), і саме листя, а не стелю пристрою: «стеля —
 *     опублікована межа пристрою, і підганяти її під те, що не влізло,
 *     означало б скасувати перевірку, а не пройти її».
 *
 * Тобто тест стеріг межу, ЖОРСТКІШУ за ту, яку рушій собі поставив, і
 * падав на дереві, що в свою межу вкладається. Виміряно сьогодні на
 * фікстурі: 148 згустків, 669 листків, обрізано 0.
 */
const MAX_FOLIAGE_CLUSTERS = 220;
const MAX_FOLIAGE_LEAVES = 700;
const MAX_MEDIUM_LEAF_INSTANCES = 720;
const MAX_MEDIUM_LIFE_PROFILES = 720;
const MAX_TREE_TRIANGLES = 18_000;
