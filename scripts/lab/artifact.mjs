...
try {
  ...
  if (species === 'reef') {
    ...
    /*
     * ЧОМУ ТУТ НЕМАЄ МІРКИ КОЛЬОРУ ПО ПІКСЕЛЯХ — І ЦЕ ВИСНОВОК, А НЕ
     * НЕДОРОБКА.
     *
     * ADR-0182 §3 лишив це відкритим: «колір знімається пікселями з
     * живої сцени, а не з плану». Спроба зроблена, і вона ПРОВАЛИЛАСЬ
     * двічі поспіль — обидва рази повчально (ADR-0187).
     *
     * 1. Маскою взяли рамку купола — і міряли ВОДУ. Коробка навколо
     *    еліпсоїда містить кути, а в кутах вода й пісок: кути дали
     *    насиченість 0.26…0.35, центр самого купола — 0.16. «Живого
     *    кольору 43.7%» на рифі першого року описувало фон.
     *
     * 2. Маску виправили на еліпс (проєкція еліпсоїда є еліпсом точно),
     *    і число стало чесним — та ранжувало НАВПАКИ: рік 1, який на
     *    кадрі гола брила, дав 28.0% «живого», а рік 25, укритий
     *    колоніями на 175%, — 18.2%.
     *
     * Причина не в мірці, а в питанні. «Живий» для цього виду означає
     * «вкритий колоніями», а колонії носять ВІДТІНОК ПАРИ й навмисно
     * приглушені (ADR-0183 вибілює їх за наповненістю). Тобто літопис
     * тут за побудовою ближчий до каменю за насиченістю, ніж строката
     * дрібнота, яка літописом не є. Насиченість міряє дрібноту, а не те,
     * що питали.
     *
     * Розділяє камінь і колонію ЯСКРАВІСТЬ — і вона вже виміряна з
     * палітри, без жодного пікселя: 4.64 проти каменю в темній темі,
     * 2.00 у світлій (ADR-0183). Тобто потрібне число вже є, і знімати
     * його з растру нема за чим.
     *
     * Рамку купола лабораторія віддає й далі (`data-reef-screen-box`):
     * вона перевірена й правильна — камера вписує риф у кадр, тож рамка
     * однакова на першому й двадцять п'ятому роках, — і чекає на мірку,
     * якій маска справді знадобиться.
     */
